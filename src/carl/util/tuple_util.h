...
auto tuple_cat(Tuple1&& t1, Tuple2&& t2) {
    return detail::tuple_cat_impl(std::forward<Tuple1>(t1), std::forward<Tuple2>(t2),
...
auto tuple_apply(F&& f, Tuple&& t) {
    return detail::tuple_apply_impl(std::forward<F>(f), std::forward<Tuple>(t),
...
auto tuple_foreach(F&& f, Tuple&& t) {
    return detail::tuple_foreach_impl(std::forward<F>(f), std::forward<Tuple>(t),
                                      std::make_index_sequence<std::tuple_size<typename std::decay<Tuple>::type>::value>{});
}
