...
struct Node {
    ...
    Node(std::size_t _id, T&& _data, std::size_t _parent, std::size_t _depth) : id(_id), data(std::move(_data)), parent(_parent), depth(_depth) {}
};
template<typename T>
...
struct PreorderIterator : BaseIterator<T, PreorderIterator<T, reverse>, reverse>, std::iterator<std::bidirectional_iterator_tag, T, std::size_t, T*, T&> {
 ...
 */
template<typename T, bool reverse = false>
struct PostorderIterator : BaseIterator<T, PostorderIterator<T, reverse>, reverse>, std::iterator<std::bidirectional_iterator_tag, T, std::size_t, T*, T&> {
 ...
 */
template<typename T, bool reverse = false>
struct LeafIterator : BaseIterator<T, LeafIterator<T, reverse>, reverse>, std::iterator<std::bidirectional_iterator_tag, T, std::size_t, T*, T&> {
 ...
 */
template<typename T, bool reverse = false>
struct DepthIterator : BaseIterator<T, DepthIterator<T, reverse>, reverse>, std::iterator<std::bidirectional_iterator_tag, T, std::size_t, T*, T&> {
 ...
 */
template<typename T, bool reverse = false>
struct ChildrenIterator : BaseIterator<T, ChildrenIterator<T, reverse>, reverse>, std::iterator<std::bidirectional_iterator_tag, T, std::size_t, T*, T&> {
    ...
    ChildrenIterator(const BaseIterator<T, It, reverse>& ii) : Base(ii), parent(MAXINT) {
        if (this->mTree->is_valid(ii))
            parent = this->nodes()[ii.current].parent;
    }
 ...
 */
template<typename T>
struct PathIterator : BaseIterator<T, PathIterator<T>, false>, std::iterator<std::forward_iterator_tag, T, std::size_t, T*, T&> {
...
class tree {
   ...
   public:
    ...
    iterator setRoot(T&& data) {
        if (nodes.empty())
            nodes.emplace_back(0, std::move(data), MAXINT, 0);
        else
            nodes[0].data = data;
    ...
    Iterator append(Iterator position, tree&& data) {
        ...
        if (id > 0)
            r->previousSibling = &(position.current->children.back());
        position.current->children.push_back(*r);
        if (id > 0)
            r->previousSibling->nextSibling = &(position.current->children.back());
        assert(isConsistent());
