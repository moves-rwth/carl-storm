...
namespace traits {
template<>
inline void scale(int exp, cln::cl_RA& r) {
    if (exp >= 0)
...
namespace traits {
template<>
inline void scale(int exp, mpq_class& r) {
    if (exp >= 0)
...
struct ExpressionParser : public qi::grammar<Iterator, ExpressionType<Pol>(), Skipper> {
    ...
    class perform_addition : public boost::static_visitor<expr_type> {
       public:
        ...
        template<typename T>
        typename std::enable_if<!std::is_same<Formula<Pol>, T>::value, expr_type>::type operator()(const RatFun<Pol>& lhs, const T& rhs) const {
            return (lhs) + (rhs);
        }

        template<typename T>
        typename std::enable_if<!std::is_same<Formula<Pol>, T>::value, expr_type>::type operator()(const T& lhs, const RatFun<Pol>& rhs) const {
            return (rhs) + Pol(lhs);
        }
        ...
        template<typename T>
        typename std::enable_if<!std::is_same<Formula<Pol>, T>::value, expr_type>::type operator()(const T& lhs, const Formula<Pol>& rhs) const {
            throw std::runtime_error("No addition for formula");
        }
    };
    ...
    class perform_multiplication : public boost::static_visitor<expr_type> {
       public:
        template<typename T, typename U>
        typename std::enable_if<!std::is_same<Formula<Pol>, T>::value, expr_type>::type operator()(const T& lhs, const U& rhs) const {
            return lhs * rhs;
        }
    ...
    class perform_division : public boost::static_visitor<expr_type> {
       public:
        ...
        template<typename T>
        typename std::enable_if<!std::is_base_of<Formula<Pol>, T>::value, expr_type>::type operator()(const RatFun<Pol>& lhs, const T& rhs) const {
            return lhs / rhs;
        }
        ...
        template<typename T>
        typename std::enable_if<!std::is_same<Formula<Pol>, T>::value, expr_type>::type operator()(const T& lhs, const CoeffType& coeff) const {
    ...
    ExpressionParser() : ExpressionParser<Pol>::base_type(main, "polynomial") {
        ...
        boolean = (qi::true_[qi::_val = px::bind(&ExpressionParser<Pol>::makeBool, px::ref(*this), true)] |
                   qi::false_[qi::_val = px::bind(&ExpressionParser<Pol>::makeBool, px::ref(*this), false)]);
        ...
        expr_product =
            (expr_sign >> *(operationScaleLA > expr_power))[qi::_val = px::bind(&ExpressionParser<Pol>::arithmeticExpr, px::ref(*this), qi::_1, qi::_2)];
        expr_sum =
            (expr_product >> *(operationShift > expr_product))[qi::_val = px::bind(&ExpressionParser<Pol>::arithmeticExpr, px::ref(*this), qi::_1, qi::_2)];
        main = expr_sum;
