...
template<typename Rational, typename Poly>
unsigned satisfies(const Model<Rational, Poly>& _model, const std::map<Variable, Rational>& _assignment,
                   const std::map<carl::BVVariable, carl::BVTerm>& _bvAssigns, const Formula<Poly>& _formula) {
...
void getDefaultModel(Model<Rational, Poly>& _defaultModel, const Formula<Poly>& _formula, bool _overwrite, size_t _seed) {
    switch (_formula.getType()) {
        ...
        default:
            ...
            for (const auto& subFormula : _formula.subformulas()) getDefaultModel(_defaultModel, subFormula, _overwrite, _seed);
    }
}
...
Formula<Poly> representingFormula(const ModelVariable& mv, const ModelValue<Rational, Poly>& val) {
    if (val.isBool()) {
        ...
        if (val.isBool())
            return Formula<Poly>(mv.asVariable());
        else
            return Formula<Poly>(FormulaType::NOT, Formula<Poly>(mv.asVariable()));
