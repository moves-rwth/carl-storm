...
SqrtEx<Poly>::SqrtEx() : mConstantPart(), mFactor(), mDenominator(1), mRadicand() {}

template<typename Poly>
SqrtEx<Poly>::SqrtEx(Poly&& _poly) : mConstantPart(std::move(_poly)), mFactor(), mDenominator(1), mRadicand() {
    normalize();
}
...
bool SqrtEx<Poly>::operator==(const SqrtEx& _toCompareWith) const {
    return mRadicand == _toCompareWith.radicand() && mDenominator == _toCompareWith.denominator() && mFactor == _toCompareWith.factor() &&
           mConstantPart == _toCompareWith.constantPart();
}
...
SqrtEx<Poly> SqrtEx<Poly>::operator+(const SqrtEx<Poly>& rhs) const {
    ...
    SqrtEx<Poly> result = SqrtEx<Poly>(rhs.denominator() * constantPart() + rhs.constantPart() * denominator(),
                                       rhs.denominator() * factor() + rhs.factor() * denominator(), denominator() * rhs.denominator(), radicand());
    return result;
}
...
SqrtEx<Poly> SqrtEx<Poly>::operator*(const SqrtEx<Poly>& rhs) const {
    ...
    SqrtEx<Poly> result = SqrtEx<Poly>(rhs.constantPart() * constantPart() + rhs.factor() * factor() * radicand(),
                                       rhs.constantPart() * factor() + rhs.factor() * constantPart(), denominator() * rhs.denominator(), radicand());
    return result;
}
...
SqrtEx<Poly> SqrtEx<Poly>::operator/(const SqrtEx<Poly>& rhs) const {
    assert(!rhs.hasSqrt());
    SqrtEx<Poly> result = SqrtEx<Poly>(constantPart() * rhs.denominator(), factor() * rhs.denominator(), denominator() * rhs.factor(), radicand());
    return result;
}
...
SqrtEx<Poly> SqrtEx<Poly>::substitute(const std::map<Variable, Rational>& _evalMap) const {
    ...
    if (radicandEvaluated.isConstant() && carl::sqrt_exact(radicandEvaluated.constantPart(), sqrtExValue)) {
        return SqrtEx<Poly>(Poly(constantPartEvaluated + factorEvaluated * sqrtExValue), constant_zero<Poly>::get(), std::move(denomEvaluated),
