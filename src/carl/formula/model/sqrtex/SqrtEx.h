...
class SqrtEx {
   ...
   public:
    ...
    explicit SqrtEx(const Poly& _poly) : SqrtEx::SqrtEx(Poly(_poly)) {}

    explicit SqrtEx(Variable::Arg _var) : SqrtEx(makePolynomial<Poly>(_var)) {}
...
struct hash<carl::SqrtEx<Poly>> {
   public:
    ...
    std::size_t operator()(const carl::SqrtEx<Poly>& _sqrtEx) const {
        return ((hash<Poly>()(_sqrtEx.radicand()) ^ hash<Poly>()(_sqrtEx.denominator())) ^ hash<Poly>()(_sqrtEx.factor())) ^
               hash<Poly>()(_sqrtEx.constantPart());
    }
};
}  // namespace std
