...
class BVConstraint {
   ...
   private:
    ...
    explicit BVConstraint(bool _consistent = true) : mRelation(_consistent ? BVCompareRelation::EQ : BVCompareRelation::NEQ) {}
    ...
    BVConstraint(const BVCompareRelation& _relation, const BVTerm& _lhs, const BVTerm& _rhs)
        : mHash((toId(_relation) << 10) ^ (_lhs.hash() << 5) ^ _rhs.hash()), mRelation(_relation), mLhs(_lhs), mRhs(_rhs) {
   ...
   public:
    ...
    bool operator==(const BVConstraint& _other) const {
        ...
        return mRelation == _other.mRelation && mLhs == _other.mLhs && mRhs == _other.mRhs;
    }
    bool operator<(const BVConstraint& _other) const {
        if (mId != 0 && _other.mId != 0) {
            return mId < _other.mId;
        }
        if (mRelation != _other.mRelation)
            return mRelation < _other.mRelation;
        if (!(mLhs == _other.mLhs))
            return mLhs < _other.mLhs;
        if (!(mRhs == _other.mRhs))
            return mRhs < _other.mRhs;
        return false;
    }
