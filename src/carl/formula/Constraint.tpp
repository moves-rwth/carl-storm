...
ConstraintContent<Pol>::ConstraintContent() : mHash(CONSTRAINT_HASH(mLhs, mRelation, Pol)) {}

template<typename Pol>
ConstraintContent<Pol>::ConstraintContent(Pol&& _lhs, Relation _rel, std::size_t _id)
    : mID(_id), mRelation(_rel), mLhs(std::move(_lhs)), mHash(CONSTRAINT_HASH(mLhs, mRelation, Pol)) {
    ...
    initLazy();  // TODO: this must unfortunately be done here and basically collects the variables. The reason is that without the variables, we cannot do the
...
unsigned ConstraintContent<Pol>::isConsistent() const {
    if (mLhs.isConstant())
        ...
        switch (mRelation) {
            case Relation::EQ: {
                if (mLhsDefinitess == Definiteness::POSITIVE || mLhsDefinitess == Definiteness::NEGATIVE)
                    return 0;
                break;
            }
            case Relation::NEQ: {
            ...
            case Relation::LESS: {
                if (mLhsDefinitess == Definiteness::NEGATIVE)
                    return 1;
            ...
            case Relation::GREATER: {
                if (mLhsDefinitess == Definiteness::POSITIVE)
                    return 1;
        ...
        switch (rel) {
            ...
            case Relation::LEQ:
                if (mLhsDefinitess == Definiteness::NEGATIVE_SEMI)
                    ...
                    return new ConstraintContent((mLhs.trailingTerm().coeff() > 0 ? PolyT(typename Pol::NumberType(1)) : PolyT(typename Pol::NumberType(-1))) *
                                                     PolyT(*mVariables.begin()),
                                                 rel);
            case Relation::GEQ:
                if (mLhsDefinitess == Definiteness::POSITIVE_SEMI)
                ...
                if (mLhsDefinitess == Definiteness::NEGATIVE_SEMI)
                    ...
                    if (mLhsDefinitess == Definiteness::POSITIVE_SEMI)
                        ...
                        return new ConstraintContent(
                            (mLhs.trailingTerm().coeff() > 0 ? PolyT(typename Pol::NumberType(1)) : PolyT(typename Pol::NumberType(-1))) *
                                PolyT(*mVariables.begin()),
                            rel);
                }
            case Relation::GREATER:
...
template<typename Pol>
std::string ConstraintContent<Pol>::toString(unsigned _unequalSwitch, bool _infix, bool _friendlyVarNames) const {
    std::string result = "";
    if (_infix)
        ...
        case Relation::NEQ:
            if (_infix) {
                ...
                if (_unequalSwitch == 0)  // standard case
                    ...
                    std::string lhsString = mLhs.toString(false, _friendlyVarNames);
...
template<typename Pol>
Constraint<Pol>::Constraint(const ConstraintContent<Pol>* _content) : mpContent(_content) {
    ConstraintPool<Pol>::getInstance().reg(mpContent);
}

template<typename Pol>
Constraint<Pol>::Constraint(bool _valid) : Constraint(ConstraintPool<Pol>::getInstance().create(_valid)) {}
...
template<typename Pol>
Constraint<Pol>::Constraint(const Pol& _lhs, Relation _rel) : Constraint(ConstraintPool<Pol>::getInstance().create(_lhs, _rel)) {
    CARL_LOG_DEBUG("carl.formula.constraint", _lhs << " " << _rel << " 0  ->  " << *this);
}

template<typename Pol>
template<typename P, EnableIf<needs_cache<P>>>
Constraint<Pol>::Constraint(const typename P::PolyType& _lhs, Relation _rel) : Constraint(ConstraintPool<Pol>::getInstance().create(_lhs, _rel)) {}

template<typename Pol>
Constraint<Pol>::Constraint(const Constraint<Pol>& _constraint) : mpContent(_constraint.mpContent) {
    ConstraintPool<Pol>::getInstance().reg(mpContent);
}

template<typename Pol>
Constraint<Pol>::Constraint(Constraint<Pol>&& _constraint) noexcept : mpContent(_constraint.mpContent) {
...
bool Constraint<Pol>::getSubstitution(Variable& _substitutionVariable, Pol& _substitutionTerm, bool _negated, const Variable& _exclude) const {
    if ((!_negated && relation() != Relation::EQ) || (_negated && relation() != Relation::NEQ))
        return false;
    VARINFOMAP_LOCK_GUARD
    for (typename std::map<Variable, VarInfo<Pol>>::iterator varInfoPair = mpContent->mVarInfoMap.begin(); varInfoPair != mpContent->mVarInfoMap.end();
         ++varInfoPair) {
        if (varInfoPair->first == _exclude)
            continue;
...
bool Constraint<Pol>::getAssignment(Variable& _substitutionVariable, typename Pol::NumberType& _substitutionValue) const {
    ...
    if (lhs().nrTerms() > 2)
        return false;
    if (lhs().nrTerms() == 2 && !lhs().trailingTerm().isConstant())
        return false;
    if (!lhs().lterm().isSingleVariable())
        return false;
...
template<typename Pol>
std::ostream& operator<<(std::ostream& _out, const Constraint<Pol>& _constraint) {
