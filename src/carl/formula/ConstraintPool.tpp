        ...
        if (carl::isNegative(_lhs.lcoeff())) {
            ...
            switch (_rel) {
                ...
                case Relation::LEQ:
                    ...
                    break;
                default:
                    assert(_rel == Relation::GEQ);
                    _rel = Relation::LEQ;
                    break;
            }
        }
        CARL_LOG_DEBUG("carl.formula.constraint",
                       "Rewriting to bound: " << _lhs.getSingleVariable() << " " << _rel << " " << (-_lhs.constantPart()) / _lhs.lcoeff());
...
ConstraintContent<Pol>* ConstraintPool<Pol>::createNormalizedConstraint(const Pol& _lhs, const Relation _rel) const {
    ...
    if (_rel == Relation::GREATER) {
        ...
        if (_rel == Relation::EQ || _rel == Relation::NEQ) {
            if (!_lhs.isZero() && lhs.lterm().coeff() < typename Pol::NumberType(0))
                lhs = -lhs;
...
const ConstraintContent<Pol>* ConstraintPool<Pol>::addConstraintToPool(ConstraintContent<Pol>* _constraint) {
    ...
    //        cout << *_constraint << " is consistent: " << constraintConsistent << std::endl;
...
void ConstraintPool<Pol>::print(std::ostream& _out) const {
    ...
    for (auto constraint = mConstraints.begin(); constraint != mConstraints.end(); ++constraint)
        _out << "    " << **constraint << "  [id=" << (*constraint)->mID << ", hash=" << (*constraint)->hash() << ", usages=" << (*constraint)->mUsages << "]"
             << std::endl;
    _out << "---------------------------------------------------" << std::endl;
}
}  // namespace carl
