...
class ConstraintContent {
   ...
   private:
     ...
     *             and b is a rational.
     */
    ConstraintContent(const Pol& _lhs, Relation _rel, std::size_t _id = 0) : ConstraintContent<Pol>::ConstraintContent(std::move(Pol(_lhs)), _rel, _id) {}
...
#define FACTORIZATION_UNLOCK
   ...
   public:
    ...
    uint minDegree(const Variable& _variable) const {
        ...
        if (varInfo == mpContent->mVarInfoMap.end())
            return 0;
    ...
    bool isBound(bool negated = false) const {
        if (mpContent->mVariables.size() != 1 || maxDegree(*mpContent->mVariables.begin()) != 1)
            return false;
        if (negated) {
    ...
    bool isLowerBound() const {
        if (isBound()) {
            if (mpContent->mRelation == Relation::EQ)
                return true;
     ...
     *         false, otherwise.
     */
    bool getSubstitution(Variable& _substitutionVariable, Pol& _substitutionTerm, bool _negated = false,
...
signed compare(const Constraint<Pol>& _constraintA, const Constraint<Pol>& _constraintB) {
    ...
    bool signsDiffer =
        (one_divided_by_a < carl::constant_zero<typename Pol::NumberType>::get()) != (one_divided_by_b < carl::constant_zero<typename Pol::NumberType>::get());
