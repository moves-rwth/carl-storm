...
const FormulaContent<Pol>* FormulaPool<Pol>::add(FormulaContent<Pol>* _element) {
    ...
    {
        CARL_LOG_DEBUG("carl.formula",
                       "Just added " << static_cast<const void*>(_element) << " / " << static_cast<const void*>(*iterBoolPair.first) << " to the pool");
    ...
    for (; pos < _subformulas.size();) {
        if (pos < _subformulas.size() - 1) {
            if (_subformulas[pos] == _subformulas[pos + 1]) {
                ...
                ++pos;
                while (pos < _subformulas.size() - 1 && _subformulas[pos] == _subformulas[pos + 1]) ++pos;
...
const FormulaContent<Pol>* FormulaPool<Pol>::createITE(Formulas<Pol>&& _subformulas) {
    ...
    if (condition.isTrue())
        return thencase.mpContent;
    if (condition.isFalse())
        return elsecase.mpContent;
    ...
    if (condition == elsecase)
        elsecase = Formula<Pol>(falseFormula());
    if (condition.mpContent == elsecase.mpContent->mNegation)
        elsecase = Formula<Pol>(trueFormula());
    if (condition == thencase)
        thencase = Formula<Pol>(trueFormula());
    if (condition.mpContent == thencase.mpContent->mNegation)
        thencase = Formula<Pol>(falseFormula());
