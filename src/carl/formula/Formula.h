...
class Formula {
   ...
   private:
    ...
    explicit Formula(const FormulaContent<Pol>* _content) : mpContent(_content) {
        if (_content != nullptr)
...
#define COLLECT_VARIABLES_LOCK_GUARD
   ...
   public:
    ...
    explicit Formula(FormulaType _type = FALSE) : Formula(FormulaPool<Pol>::getInstance().create(_type)) {}

    explicit Formula(Variable::Arg _booleanVar) : Formula(FormulaPool<Pol>::getInstance().create(_booleanVar)) {}

    explicit Formula(const Pol& _pol, Relation _rel) : Formula(FormulaPool<Pol>::getInstance().create(Constraint<Pol>(_pol, _rel))) {}

    explicit Formula(const Constraint<Pol>& _constraint) : Formula(FormulaPool<Pol>::getInstance().create(_constraint)) {}

    explicit Formula(const VariableComparison<Pol>& _variableComparison) : Formula(FormulaPool<Pol>::getInstance().create(_variableComparison)) {}

    explicit Formula(const VariableAssignment<Pol>& _variableAssignment) : Formula(FormulaPool<Pol>::getInstance().create(_variableAssignment)) {}

    explicit Formula(const BVConstraint& _constraint) : Formula(FormulaPool<Pol>::getInstance().create(_constraint)) {}

    explicit Formula(const PBConstraint<Pol>& _constraint) : Formula(FormulaPool<Pol>::getInstance().create(_constraint)) {}

    explicit Formula(FormulaType _type, Formula&& _subformula) : Formula(FormulaPool<Pol>::getInstance().create(_type, std::move(_subformula))) {}

    explicit Formula(FormulaType _type, const Formula& _subformula) : Formula(FormulaPool<Pol>::getInstance().create(_type, std::move(Formula(_subformula)))) {}
    ...
    explicit Formula(FormulaType _type, const Formula& _subformulaA, const Formula& _subformulaB)
        ...
        assert(_type == FormulaType::AND || _type == FormulaType::IFF || _type == FormulaType::IMPLIES || _type == FormulaType::OR ||
               _type == FormulaType::XOR);
    }
    ...
    explicit Formula(FormulaType _type, const FormulasMulti<Pol>& _subformulas) : Formula(FormulaPool<Pol>::getInstance().create(_subformulas)) {
        assert(_type == FormulaType::XOR);
    }

    explicit Formula(FormulaType _type, const Formulas<Pol>& _subasts) : Formula(FormulaPool<Pol>::getInstance().create(_type, _subasts)) {}

    explicit Formula(FormulaType _type, Formulas<Pol>&& _subasts) : Formula(FormulaPool<Pol>::getInstance().create(_type, std::move(_subasts))) {}
    ...
    explicit Formula(UEquality&& _eq) : Formula(FormulaPool<Pol>::getInstance().create(std::move(_eq))) {}

    explicit Formula(const UEquality& _eq) : Formula(FormulaPool<Pol>::getInstance().create(std::move(UEquality(_eq)))) {}

    explicit Formula(PBConstraint<Pol>&& _pbc) : Formula(FormulaPool<Pol>::getInstance().create(std::move(_pbc))) {}

    Formula(const Formula& _formula) : mpContent(_formula.mpContent) {
        if (_formula.mpContent != nullptr)
            FormulaPool<Pol>::getInstance().reg(_formula.mpContent);
    }

    Formula(Formula&& _formula) noexcept : mpContent(_formula.mpContent) {
    ...
    bool empty() const {
        if (mpContent->mType == FormulaType::BOOL || mpContent->mType == FormulaType::CONSTRAINT || mpContent->mType == FormulaType::TRUE ||
            mpContent->mType == FormulaType::FALSE || mpContent->mType == FormulaType::BITVECTOR)
            return false;
        else
#ifdef __VS
    ...
    const_iterator end() const {
        assert(mpContent->mType == FormulaType::AND || mpContent->mType == FormulaType::OR || mpContent->mType == FormulaType::IFF ||
     ...
     */
    bool isAtom() const {
        return (mpContent->mType == FormulaType::CONSTRAINT || mpContent->mType == FormulaType::BOOL || mpContent->mType == FormulaType::VARCOMPARE ||
                mpContent->mType == FormulaType::VARASSIGN || mpContent->mType == FormulaType::UEQ || mpContent->mType == FormulaType::BITVECTOR ||
    ...
    bool isBound() const {
        ...
        if (mpContent->mType == FormulaType::CONSTRAINT)
            return mpContent->mpConstraintVS->isBound();
        if (mpContent->mType == FormulaType::NOT) {
            if (mpContent->mType != FormulaType::CONSTRAINT)
                return false;
...
#else
        if (mpContent->mType == FormulaType::CONSTRAINT)
            return mpContent->mConstraint.isBound();
        if (mpContent->mType == FormulaType::NOT) {
            if (mpContent->mSubformula.mpContent->mType != FormulaType::CONSTRAINT)
                return false;
    ...
    bool isOnlyPropositional() const {
        return !(carl::PROP_CONTAINS_BITVECTOR <= properties()) && !(carl::PROP_CONTAINS_UNINTERPRETED_EQUATIONS <= properties()) &&
               !(carl::PROP_CONTAINS_INTEGER_VALUED_VARS <= properties()) && !(carl::PROP_CONTAINS_REAL_VALUED_VARS <= properties()) &&
    ...
    void collectVariables_(Variables& _vars, std::set<BVVariable>* _bvVars, std::set<UVariable>* _ueVars, bool _booleanVars, bool _realVars, bool _integerVars,
                           bool _uninterpretedVars, bool _bitvectorVars) const;
   ...
   public:
     ...
     */
    std::string toString(bool _withActivity = false, unsigned _resolveUnequal = 0, const std::string _init = "", bool _oneline = true, bool _infix = false,
                         bool _friendlyNames = true, bool _withVariableDefinition = false) const;
     ...
     */
    std::string variableListToString(std::string _separator = ",",
                                     const std::unordered_map<std::string, std::string>& _variableIds = (std::unordered_map<std::string, std::string>())) const;
...
struct FormulaSubstitutor {
   private:
    ...
    struct BitvectorSubstitutor {
        ...
        Formula operator()(const Formula& formula) {
            if (formula.getType() != FormulaType::BITVECTOR)
                return formula;
    ...
    struct UninterpretedSubstitutor {
        ...
        Formula operator()(const Formula& formula) {
            if (formula.getType() != FormulaType::UEQ)
                return formula;
        }
    };
