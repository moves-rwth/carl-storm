...
void Formula<Pol>::collectVariables(Variables& _vars, bool _booleanVars, bool _realVars, bool _integerVars, bool _uninterpretedVars,
                                    bool _bitvectorVars) const {
...
template<typename Pol>
void Formula<Pol>::collectVariables_(Variables& _vars, std::set<BVVariable>* _bvVars, std::set<UVariable>* _ueVars, bool _booleanVars, bool _realVars,
                                     bool _integerVars, bool _uninterpretedVars, bool _bitvectorVars) const {
    switch (getType()) {
...
size_t Formula<Pol>::complexity() const {
    ...
    visitor.visit(*this, [&](const Formula& _f) {
        switch (_f.getType()) {
            case FormulaType::TRUE:
            case FormulaType::FALSE:
                break;
            case FormulaType::CONSTRAINT:
                result += _f.constraint().complexity();
                break;
            case FormulaType::BITVECTOR:
                result += _f.bvConstraint().complexity();
                break;
            case FormulaType::UEQ:
                result += _f.uequality().complexity();
                break;
            default:
                ++result;
        }
...
template<typename Pol>
std::string Formula<Pol>::toString(bool _withActivity, unsigned _resolveUnequal, const std::string _init, bool _oneline, bool _infix, bool _friendlyNames,
                                   bool _withVariableDefinition) const {
    std::string result = "";
    if (_withVariableDefinition) {
        ...
        for (auto var : vars) os << "(declare-fun " << var << " () " << var.type() << ")\n";
        for (const auto& uvar : uvars) os << "(declare-fun " << uvar() << " () " << uvar.domain() << ")\n";
        for (const auto& bvvar : bvvars) os << "(declare-fun " << bvvar << " () " << bvvar.sort() << ")\n";
...
string Formula<Pol>::toRedlogFormat(bool _withVariables) const {
    ...
    switch (getType()) {
        ...
        default: {
            ...
            if (_withVariables) {  // add the variables
                result += "( ex( {";
...
Formula<Pol> Formula<Pol>::resolveNegation(bool _keepConstraint) const {
    ...
    switch (subformula().getType()) {
        ...
        case FormulaType::XOR:  // (not (xor phi_1 .. phi_n))  ->  (xor (not phi_1) phi_2 .. phi_n)
        {
            ...
            ++subFormula;
            for (; subFormula != subformula().subformulas().end(); ++subFormula) subFormulas.push_back(*subFormula);
    ...
    Formulas<Pol> subFormulas;
    for (const Formula<Pol>& subsubformula : subformula().subformulas()) subFormulas.push_back(Formula<Pol>(FormulaType::NOT, subsubformula));
...
Formula<Pol> Formula<Pol>::connectPrecedingSubformulas() const {
    ...
    if (subformulas().size() > 2) {
        ...
        ++iter;
        for (; iter != subformulas().rend(); ++iter) tmpSubformulas.push_back(*iter);
    ...
    switch (getType()) {
        ...
        case FormulaType::XOR: {
            if (!negated) {
                ...
                if (getType() == FormulaType::AND)
                    return Formula<Pol>(FormulaType::OR, std::move(subs));
                else
                    return Formula<Pol>(FormulaType::AND, std::move(subs));
        ...
        case FormulaType::FORALL: {
            unsigned cur = 0;
            if ((level % 2 == (getType() == FormulaType::EXISTS ? (unsigned)0 : (unsigned)1)) ^ negated)
                cur = level;
            else
                cur = level + 1;
            ...
            for (auto it = vars.begin(); it != vars.end();) {
                if (it->type() == VariableType::VT_BOOL) {
                    ...
                    if (cur > 0) {
                        f = Formula<Pol>((getType() == FormulaType::EXISTS ? FormulaType::OR : FormulaType::AND),
                                         {f.substitute({{*it, Formula<Pol>(FormulaType::TRUE)}}), f.substitute({{*it, Formula<Pol>(FormulaType::FALSE)}})});
                    }
                    it = vars.erase(it);
                } else
                    it++;
        ...
        case FormulaType::IMPLIES:
            if (negated)
                return Formula<Pol>(FormulaType::AND, {premise().toQF(variables, level, false), conclusion().toQF(variables, level, true)});
            else
                return Formula<Pol>(FormulaType::IMPLIES, {premise().toQF(variables, level, false), conclusion().toQF(variables, level, false)});
        case FormulaType::ITE:
            return Formula<Pol>(FormulaType::ITE, {condition().toQF(variables, level, negated), firstCase().toQF(variables, level, negated),
                                                   secondCase().toQF(variables, level, negated)});
        case FormulaType::NOT:
...
Formula<Pol> Formula<Pol>::toCNF(bool _keepConstraints, bool _simplifyConstraintCombinations, bool _tseitinWithEquivalence) const {
    ...
    while (!subformulasToTransform.empty()) {
        ...
        switch (currentFormula.getType()) {
            ...
            {
                for (const Formula<Pol>& subFormula : currentFormula.subformulas()) subformulasToTransform.push_back(subFormula);
                break;
            }
            ...
            case FormulaType::ITE:  // (ite cond then else)  ->  auxBool, where (or (not cond) then) and (or cond else) are added to the queue
            {
                ...
                subformulasToTransform.push_back(
                    Formula<Pol>(FormulaType::OR, {Formula<Pol>(FormulaType::NOT, currentFormula.condition()), currentFormula.firstCase()}));
                // Add: (or cond else)
            ...
            case FormulaType::IFF: {
                if (currentFormula.subformulas().size() > 2) {
                    ...
                    subformulasToTransform.push_back(
                        Formula<Pol>(FormulaType::OR, {Formula<Pol>(AND, move(subformulasA)), Formula<Pol>(FormulaType::AND, move(subformulasB))}));
                } else {
            ...
            case FormulaType::OR:  // (or phi_1 .. phi_n) -> (or psi_1 .. psi_m),  where phi_i is transformed as follows:
            {
                ...
                for (const Formula<Pol>& subFormula : currentFormula.subformulas()) phis.push_back(subFormula);
                ...
                while (!currentFormulaValid && !phis.empty()) {
                    ...
                    switch (currentSubformula.getType()) {
                        ...
                        case FormulaType::OR:  // (or phi_i1 .. phi_ik) -> phi_i1 .. phi_ik
                            for (const Formula<Pol>& subsubformula : currentSubformula.subformulas()) phis.push_back(subsubformula);
                            break;
                        ...
                        case FormulaType::ITE:  // (ite cond then else)  ->  (and (or (not cond) then) (or cond else))
                        {
                            Formulas<Pol> tmpSubformulas;
                            // Add: (or (not cond) then)
                            tmpSubformulas.push_back(
                                Formula<Pol>(FormulaType::OR, {Formula<Pol>(FormulaType::NOT, currentSubformula.condition()), currentSubformula.firstCase()}));
                            // Add: (or cond else)
    ...
    while (!_constraintBounds.empty()) {
        ...
        if (bounds.size() == 1) {
            ...
            for (; iter != bounds.end(); ++iter) {
...
#endif
                if ((_inConjunction && iter->second.first == Relation::NEQ) || (!_inConjunction && iter->second.first == Relation::EQ)) {
                    if (moreSignificantCase == bounds.end()) {
                        if ((_inConjunction && mostSignificantUpperBound == bounds.end()) || (!_inConjunction && mostSignificantLowerBound == bounds.end())) {
                ...
                } else if ((_inConjunction && iter->second.first == Relation::EQ) || (!_inConjunction && iter->second.first == Relation::NEQ)) {
            ...
            if (moreSignificantCase != bounds.end()) {
                ...
                if (!(_inConjunction || mostSignificantUpperBound == bounds.end() || mostSignificantLowerBound == bounds.end() ||
                    ...
                    cout << "mostSignificantUpperBound:   " << mostSignificantUpperBound->first << "  [" << mostSignificantUpperBound->second.second << "]"
                         << endl;
                    cout << "mostSignificantLowerBound:   " << mostSignificantLowerBound->first << "  [" << mostSignificantLowerBound->second.second << "]"
                         << endl;
                }
#endif
