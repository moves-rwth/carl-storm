...
class SMTLIBStream {
   private:
    ...
    void write(const mpz_class& n) {
        *this << carl::toString(n, false);
    }
    void write(const mpq_class& n) {
        *this << carl::toString(n, false);
    }
#ifdef USE_CLN_NUMBERS
    void write(const cln::cl_I& n) {
        *this << carl::toString(n, false);
    }
    void write(const cln::cl_RA& n) {
        *this << carl::toString(n, false);
    }
#endif
    ...
    void write(const Monomial& m) {
        if (m.exponents().empty())
            *this << "1";
        else if (m.exponents().size() == 1)
            *this << m.exponents().front();
        else {
            *this << "(*";
            ...
            *this << "(+";
            for (std::size_t i = 0; i < up.coefficients().size(); i++) {
                ...
                if (exp == 0)
                    ...
                    *this << " (* " << coeff << " " << Monomial(up.mainVar(), exp) << ")";
            }
            *this << ")";
        }
    }
...
struct SMTLIBScriptContainer {
    ...
    SMTLIBScriptContainer(Logic l, std::initializer_list<Formula<Pol>> f, const Pol& objective, bool getModel = false)
        : mLogic(l), mFormulas(f), mGetModel(getModel), mObjective(objective) {}
};
template<typename Pol>
std::ostream& operator<<(std::ostream& os, const SMTLIBScriptContainer<Pol>& sc) {
    ...
    if (!sc.mObjective.isZero())
        sls.minimize(sc.mObjective);
    sls.checkSat();
    if (sc.mGetModel)
        sls.getModel();
    return os << sls;
