...
struct NamedAssertion {
    NamedAssertion(const SMT2AssertionName& assertion_name) : name(assertion_name) {}
   ...
   public:
    ...
    WriteTosmt2Stream& operator<<(smt2node n) {
        switch (n) {
            case smt2node::CLOSENODE: {
                if (level == 0)
                    throw std::invalid_argument("Cannot close a node, no node open");
                --level;
    ...
    WriteTosmt2Stream& operator<<(const Constraint<RationalFunction<Pol>> c) {
        ...
        if (c.lhs().denominator() == Pol(1)) {
            ...
            mAssertions.back()->assertion << "(" << c.rel() << " (/ " << c.lhs().nominator().toString(false, true) << " "
                                          << c.lhs().denominator().toString(false, true) << ") 0)";
        }
        if (mAutoLineBreaks)
            mAssertions.back()->assertion << "\n";
        return *this;
