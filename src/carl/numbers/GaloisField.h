...
class GaloisField {
   ...
   public:
     ...
     */
    explicit GaloisField(BaseIntType p, BaseIntType k = 1) : mP(p), mK(k), mPK(pow(p, k)), mMaxValue((mPK - 1) / 2), mModulus(mMaxValue + 1) {}
   ...
   public:
    const GaloisField<IntegerType>* getField(BaseIntType p, BaseIntType k = 1) {
        ...
        if (it == mGaloisFields.end()) {
            auto newit = mGaloisFields.emplace(std::make_pair(p, k), std::make_unique<GaloisField<IntegerType>>(p, k));
            assert(newit.second);
