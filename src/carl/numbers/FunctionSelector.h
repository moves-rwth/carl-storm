 ...
 *   For most carl classes and the use case we have in mind, relying on carl::UnderlyingNumberType will work. Hence, there is NaryTypeSelector which will use
 * only the first argument to deduce the type. Note there is no check that all other arguments (if there are any) would yield the same type!
 * @code{.cpp}
