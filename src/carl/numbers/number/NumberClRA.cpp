...
Number<cln::cl_RA>::Number(const Number<cln::cl_I>& a, const Number<cln::cl_I>& b) : Number(cln::cl_RA(a) / b) {}
...
std::string Number<cln::cl_RA>::toString(bool _infix) const {
    ...
    if (_infix)
        ...
        if (mData != carl::constant_one<cln::cl_I>().get())
            s << "(/ " << cln::abs(cln::numerator(mData)) << " " << cln::abs(d) << ")";
        else
            s << this->abs().getValue();
    }
    if (negative)
