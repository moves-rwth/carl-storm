...
class Number<cln::cl_RA> : public BaseNumber<cln::cl_RA, Number<cln::cl_RA>> {
   ...
   public:
    ...
    explicit Number(double d) {
        mData = cln::rationalize(d);
    }
    explicit Number(float f) {
        mData = cln::rationalize(f);
    }
     ...
     * Note that we try to find bounds that are very close to the actual square root. If a small representation is more important than a small interval,
     * sqrt_fast should be used.
