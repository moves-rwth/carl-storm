...
template<typename Coeff, typename Ordering, typename Policies>
MultivariatePolynomial<Coeff, Ordering, Policies>::MultivariatePolynomial() : mTerms(), mOrdered(true) {
...
MultivariatePolynomial<Coeff, Ordering, Policies>::MultivariatePolynomial(const MultivariatePolynomial<Coeff, Ordering, Policies>& p)
    : Policies(p), mTerms(p.mTerms), mOrdered(p.isOrdered()) {
    assert(this->isConsistent());
}

template<typename Coeff, typename Ordering, typename Policies>
MultivariatePolynomial<Coeff, Ordering, Policies>::MultivariatePolynomial(MultivariatePolynomial<Coeff, Ordering, Policies>&& p)
    : Policies(p), mTerms(std::move(p.mTerms)), mOrdered(p.isOrdered()) {
    assert(this->isConsistent());
}

template<typename Coeff, typename Ordering, typename Policies>
MultivariatePolynomial<Coeff, Ordering, Policies>& MultivariatePolynomial<Coeff, Ordering, Policies>::operator=(
...
MultivariatePolynomial<Coeff, Ordering, Policies>::MultivariatePolynomial(const Coeff& c)
    : Policies(), mTerms(c == Coeff(0) ? 0 : 1, Term<Coeff>(c)), mOrdered(true) {
    assert(this->isConsistent());
}

template<typename Coeff, typename Ordering, typename Policies>
MultivariatePolynomial<Coeff, Ordering, Policies>::MultivariatePolynomial(Variable::Arg v) : Policies(), mTerms(1, Term<Coeff>(v)), mOrdered(true) {
    assert(this->isConsistent());
}

template<typename Coeff, typename Ordering, typename Policies>
MultivariatePolynomial<Coeff, Ordering, Policies>::MultivariatePolynomial(const Monomial::Arg& m) : Policies(), mTerms(1, Term<Coeff>(m)), mOrdered(true) {
    assert(this->isConsistent());
}

template<typename Coeff, typename Ordering, typename Policies>
MultivariatePolynomial<Coeff, Ordering, Policies>::MultivariatePolynomial(const Term<Coeff>& t) : Policies(), mTerms(1, t), mOrdered(true) {
    if (t.isZero()) {
        this->mTerms.clear();
    }
    assert(this->isConsistent());
...
MultivariatePolynomial<Coeff, Ordering, Policies>::MultivariatePolynomial(const UnivariatePolynomial<MultivariatePolynomial<Coeff, Ordering, Policies>>& p)
    : Policies(), mTerms(), mOrdered(false) {
...
template<typename Coeff, typename Ordering, typename Policies>
MultivariatePolynomial<Coeff, Ordering, Policies>::MultivariatePolynomial(const UnivariatePolynomial<Coeff>& p) : Policies(), mTerms(), mOrdered(true) {
    exponent exp = 0;
    mTerms.reserve(p.degree());
    for (const auto& c : p.coefficients()) {
        if (!carl::isZero(c)) {
            if (exp == 0)
                mTerms.emplace_back(c);
            else
                mTerms.emplace_back(c, p.mainVar(), exp);
        }
        exp++;
    }
    assert(this->isConsistent());
...
MultivariatePolynomial<Coeff, Ordering, Policies>::MultivariatePolynomial(const MultivariatePolynomial<Coeff, Ordering, OtherPolicies>& pol)
    : Policies(), mTerms(pol.begin(), pol.end()), mOrdered(pol.isOrdered()) {
    assert(this->isConsistent());
}
...
MultivariatePolynomial<Coeff, Ordering, Policies>::MultivariatePolynomial(TermsType&& terms, bool duplicates, bool ordered)
    :
#else
MultivariatePolynomial<Coeff, Ordering, Policies>::MultivariatePolynomial(MultivariatePolynomial<Coeff, Ordering, Policies>::TermsType&& terms, bool duplicates,
                                                                          bool ordered)
    :
#endif
...
MultivariatePolynomial<Coeff, Ordering, Policies>::MultivariatePolynomial(const TermsType& terms, bool duplicates, bool ordered)
    :
#else
MultivariatePolynomial<Coeff, Ordering, Policies>::MultivariatePolynomial(const MultivariatePolynomial<Coeff, Ordering, Policies>::TermsType& terms,
                                                                          bool duplicates, bool ordered)
    :
#endif
      mTerms(terms),
      mOrdered(ordered) {
...
MultivariatePolynomial<Coeff, Ordering, Policies>::MultivariatePolynomial(const std::initializer_list<Term<Coeff>>& terms)
    : Policies(), mTerms(terms), mOrdered(false) {
    makeMinimallyOrdered();
    assert(this->isConsistent());
}

template<typename Coeff, typename Ordering, typename Policies>
MultivariatePolynomial<Coeff, Ordering, Policies>::MultivariatePolynomial(const std::initializer_list<Variable>& terms) : mTerms(), mOrdered(false) {
...
Definiteness MultivariatePolynomial<Coeff, Ordering, Policies>::definiteness(bool _fullEffort) const {
    ...
    if (_fullEffort && result == Definiteness::NON && totalDegree() == 2) {
        ...
        if (hasConstantTerm()) {
            ...
            if (constPartNegative != lTermNegative)
                return Definiteness::NON;
        ...
        if (lTermNegative)
            tmp = -tmp;
        if (!tmp.sosDecomposition(true).empty())
            return result;
        return Definiteness::NON;
    }
    return result;
...
void MultivariatePolynomial<Coeff, Ordering, Policies>::addTerm(const Term<Coeff>& term) {
    ...
    if (term.isConstant()) {
        if (hasConstantTerm()) {
            ...
            mTerms.insert(mTerms.begin(), term);
        return;
    }
    if (mOrdered) {
...
template<typename Coeff, typename Ordering, typename Policies>
template<typename C, EnableIf<is_field<C>>>
bool MultivariatePolynomial<Coeff, Ordering, Policies>::divideBy(const MultivariatePolynomial<Coeff, Ordering, Policies>& divisor,
                                                                 MultivariatePolynomial<Coeff, Ordering, Policies>& quotient) const {
...
void MultivariatePolynomial<Coeff, Ordering, Policies>::substituteIn(Variable::Arg var, const MultivariatePolynomial<Coeff, Ordering, Policies>& value) {
    ...
    for (const auto& term : mTerms) {
        if (term.monomial()) {  // This is not the constant part.
            exponent e = term.monomial()->exponentOfVariable(var);
            if (e > 1) {  // Variable occurs with exponent at least two. Insert into map and increase counter in map.
            ...
            } else if (e == 1) {  // Variable occurs with exponent one.
                expectedResultSize += value.nrTerms();
            } else {  // Variable does not occur in this term.
                ++expectedResultSize;
            }
        } else {  // This is the constant part.
            ++expectedResultSize;
        }
    }
    ...
    for (const auto& term : mTerms) {
        if (term.monomial() == nullptr) {
            ...
            Monomial::Arg mon;
            if (e > 0)
                mon = term.monomial()->dropVariable(var);
            if (e == 1) {
                for (auto vterm : value.mTerms) {
                    if (mon == nullptr)
                        mTermAdditionManager.template addTerm<false>(id, Term<Coeff>(vterm.coeff() * term.coeff(), vterm.monomial()));
                    else if (vterm.monomial() == nullptr)
                        mTermAdditionManager.template addTerm<false>(id, Term<Coeff>(vterm.coeff() * term.coeff(), mon));
                    else
                        mTermAdditionManager.template addTerm<false>(id, Term<Coeff>(vterm.coeff() * term.coeff(), vterm.monomial() * mon));
                }
            } else if (e > 1) {
...
template<typename Coeff, typename Ordering, typename Policies>
MultivariatePolynomial<Coeff, Ordering, Policies> MultivariatePolynomial<Coeff, Ordering, Policies>::substitute(
    Variable::Arg var, const MultivariatePolynomial<Coeff, Ordering, Policies>& value) const {
...
template<typename Coeff, typename Ordering, typename Policies>
MultivariatePolynomial<Coeff, Ordering, Policies> MultivariatePolynomial<Coeff, Ordering, Policies>::substitute(
    const std::map<Variable, MultivariatePolynomial<Coeff, Ordering, Policies>>& substitutions) const {
    MultivariatePolynomial result(*this);
...
MultivariatePolynomial<Coeff, Ordering, Policies> MultivariatePolynomial<Coeff, Ordering, Policies>::substitute(
    const std::map<Variable, SubstitutionType>& substitutions) const {
...
template<typename Coeff, typename Ordering, typename Policies>
MultivariatePolynomial<Coeff, Ordering, Policies> MultivariatePolynomial<Coeff, Ordering, Policies>::substitute(
    const std::map<Variable, Term<Coeff>>& substitutions) const {
    MultivariatePolynomial result;
    auto id = mTermAdditionManager.getId(mTerms.size());
    for (const auto& term : mTerms) {
...
void MultivariatePolynomial<Coeff, Ordering, Policies>::square() {
    ...
    for (auto it1 = mTerms.rbegin(); it1 != mTerms.rend(); it1++) {
        if (it1 == mTerms.rbegin())
            newlterm = it1->pow(2);
        else
            mTermAdditionManager.template addTerm<false>(id, it1->pow(2));
...
MultivariatePolynomial<Coeff, Ordering, Policies> MultivariatePolynomial<Coeff, Ordering, Policies>::pow(std::size_t exp) const {
    ...
    if (isZero())
        return MultivariatePolynomial(constant_zero<Coeff>::get());
    if (exp == 0)
        return MultivariatePolynomial(constant_one<Coeff>::get());
    if (exp == 1)
        return MultivariatePolynomial(*this);
    if (exp == 2)
        return *this * *this;
...
VariableInformation<gatherCoeff, MultivariatePolynomial<Coeff, Ordering, Policies>> MultivariatePolynomial<Coeff, Ordering, Policies>::getVarInfo(
...
bool operator==(const MultivariatePolynomial<C, O, P>& lhs, const MultivariatePolynomial<C, O, P>& rhs) {
    ...
    for (const auto& t : rhs.mTerms) {
        ...
        if (t.monomial())
            id = t.monomial()->id();
        if ((coeffs[id] == nullptr) || *coeffs[id] != t.coeff())
            return false;
    }
    return true;
}
...
bool operator==(const MultivariatePolynomial<C, O, P>& lhs, const Term<C>& rhs) {
    if (lhs.isZero() && carl::isZero(rhs.coeff()))
        return true;
    if (lhs.nrTerms() > 1)
        return false;
...
bool operator<(const MultivariatePolynomial<C, O, P>& lhs, const MultivariatePolynomial<C, O, P>& rhs) {
    ...
    if (lhs.totalDegree() == 0)
        return lhs.constantPart() < rhs.constantPart();
    if (lhs.lterm() < rhs.lterm())
        return true;
    if (lhs.lterm() > rhs.lterm())
        return false;
...
bool operator<(const MultivariatePolynomial<C, O, P>& lhs, const Term<C>& rhs) {
    if (lhs.isZero())
        return carl::constant_zero<C>().get() < rhs;
    return (lhs.lterm()) < rhs;
}
template<typename C, typename O, typename P>
bool operator<(const MultivariatePolynomial<C, O, P>& lhs, const std::shared_ptr<const carl::Monomial>& rhs) {
    ...
    if (mTerms.empty()) {
        ...
        if (this->hasConstantTerm()) {
            ...
            if (carl::isZero(Coeff(mTerms.front().coeff() + rhs.coeff()))) {
                ...
                mTerms.front() = Term<Coeff>(mTerms.front().coeff() + rhs.coeff(), nullptr);
        } else if (mTerms.size() == 1) {
        ...
        if (carl::isZero(Coeff(lcoeff() + rhs.coeff()))) {
            ...
            mTerms.back() = Term<Coeff>(lcoeff() + rhs.coeff(), rhs.monomial());
...
MultivariatePolynomial<Coeff, Ordering, Policies>& MultivariatePolynomial<Coeff, Ordering, Policies>::operator+=(const Coeff& rhs) {
    if (carl::isZero(rhs))
        return *this;
    if (hasConstantTerm()) {
        if (carl::isZero(Coeff(constantPart() + rhs)))
            mTerms.erase(mTerms.begin());
        else
            mTerms.front() = TermType(constantPart() + rhs);
    } else {
    ...
    if (Policies::searchLinear) {
        ...
        while (it != mTerms.end()) {
            ...
            if ((*it).monomial() != nullptr) {
                ...
                if (mOrdered && cmpres == CompareResult::GREATER)
                    break;
    ...
    for (auto t1 = mTerms.rbegin(); t1 != mTerms.rend(); t1++) {
        for (auto t2 = rhs.mTerms.rbegin(); t2 != rhs.mTerms.rend(); t2++) {
            if (first) {
                ...
                mTermAdditionManager.template addTerm<false>(id, std::move((*t1) * (*t2)));
        }
    }
    mTermAdditionManager.readTerms(id, mTerms);
    if (newlterm.isZero())
...
std::string MultivariatePolynomial<Coeff, Ordering, Policies>::toString(bool infix, bool friendlyVarNames) const {
    ...
    if (mTerms.empty())
        return "0";
    if (mTerms.size() == 1)
        return this->mTerms.front().toString(infix, friendlyVarNames);
    std::string result;
    if (!infix)
        result += "(+";
    for (auto term = mTerms.rbegin(); term != mTerms.rend(); term++) {
        if (infix) {
            if (term != mTerms.rbegin())
                result += "+";
        } else
            result += " ";
