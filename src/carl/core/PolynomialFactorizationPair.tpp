...
std::string factorizationToString(const Factorization<P>& _factorization, bool _infix, bool _friendlyVarNames) {
    if (_factorization.empty()) {
        ...
        if (_infix) {
            ...
            for (auto polyExpPair = _factorization.begin(); polyExpPair != _factorization.end(); ++polyExpPair) {
                assert(polyExpPair->second > 0);
                for (size_t i = 0; i < polyExpPair->second; ++i) result += " " + polyExpPair->first.toString(false, _friendlyVarNames);
            }
            result += ")";
        }
        return result;
...
PolynomialFactorizationPair<P>::PolynomialFactorizationPair(Factorization<P>&& _factorization, P* _polynomial)
    : mHash(0), mMutex(), mFactorization(std::move(_factorization)), mpPolynomial(_polynomial), mIrreducible(-1) {
...
template<typename P>
void PolynomialFactorizationPair<P>::setNewFactors(const FactorizedPolynomial<P>& _fpolyA, carl::exponent exponentA, const FactorizedPolynomial<P>& _fpolyB,
                                                   carl::exponent exponentB) const {
...
template<typename P>
Factorization<P> gcd(const PolynomialFactorizationPair<P>& _pfPairA, const PolynomialFactorizationPair<P>& _pfPairB, Factorization<P>& _restA,
                     Factorization<P>& _restB, typename P::CoeffType& _coeff, bool& _pfPairARefined, bool& _pfPairBRefined) {
    ...
    while (!factorizationA.empty()) {
        ...
        while (!factorA.isOne() && !factorizationB.empty()) {
            ...
            CARL_LOG_DEBUG("carl.core.factorizedpolynomial", "");
            // Check if the factors are equal (even if we removed equal factor by commonDivisor() beforehand, common factors could be introduced during this
            // loop)
            if (factorA == factorB) {
                // Common factor found
                ...
                if (exponentA > exponentCommon) {
                    ...
                    CARL_LOG_DEBUG("carl.core.factorizedpolynomial",
                                   __LINE__ << ": add (" << factorA << ")^" << (exponentA - exponentCommon) << " to first open remainder: " << factorizationA);
                }
                if (exponentB > exponentCommon) {
                    ...
                    CARL_LOG_DEBUG("carl.core.factorizedpolynomial",
                                   __LINE__ << ": add (" << factorB << ")^" << (exponentB - exponentCommon) << " to second open remainder: " << _restB);
                }
                ...
                if (polGCD.isOne()) {
                    ...
                    CARL_LOG_DEBUG("carl.core.factorizedpolynomial",
                                   __LINE__ << ": add (" << factorB << ")^" << exponentB << " to second finished remainder: " << _restB);
                } else {
                    // New common factor
                    ...
                    if (remainA.isOne()) {
                        ...
                        if (exponentA > exponentCommon) {
                            ...
                            CARL_LOG_DEBUG("carl.core.factorizedpolynomial", __LINE__ << ": add (" << gcdResult << ")^" << (exponentA - exponentCommon)
                                                                                      << " to first open remainder: " << factorizationA);
                        }
                    }
                    ...
                    if (exponentB > exponentCommon) {
                        ...
                        CARL_LOG_DEBUG("carl.core.factorizedpolynomial", __LINE__ << ": add (" << gcdResult << ")^" << (exponentB - exponentCommon)
                                                                                  << " to second open remainder: " << factorizationB);
                    }
                    ...
                    if (!remainB.isOne()) {
                        ...
                        CARL_LOG_DEBUG("carl.core.factorizedpolynomial",
                                       __LINE__ << ": add (" << polRemainB << ")^" << exponentB << " to second open remainder: " << _restB);
                    }
                }
            }
        }  // End of inner while
    ...
    CARL_LOG_DEBUG("carl.core.factorizedpolynomial",
                   "GCD (internal) of " << _pfPairA << " and " << _pfPairB << ": " << result << " with rests " << _restA << " and " << _restB);
            ...
            for (auto ft = _pfPair.mFactorization.begin(); ft != _pfPair.mFactorization.end();) {
                ...
                if (ft->first.content().isIrreducible()) {
                    ...
                    for (auto partFactor = refinement.begin(); partFactor != refinement.end(); partFactor++) {
                        ...
                        auto insertResult =
                            _pfPair.mFactorization.insert(std::pair<FactorizedPolynomial<P>, carl::exponent>(partFactor->first, partFactor->second * e));
