 ...
 * A single variable (identified by `id` and `type`) should not occur with two different `rank` values in the same context and hence such a comparison should
 * never take place.
 *
...
class Variable {
   ...
   public:
     ...
     *
     * Note that the variable order is not the order of the variable id. We consider variables greater, if they are defined earlier, i.e. if they have a smaller
     * id. Hence, the variables order and the order of the variable ids are reversed.
