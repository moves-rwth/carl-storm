...
class RationalFunction {
   ...
   public:
    RationalFunction() : mNumberQuotient(0), mIsSimplified(true) {}

    explicit RationalFunction(int v) : mNumberQuotient(v), mIsSimplified(true) {}

    explicit RationalFunction(const CoeffType& c) : mNumberQuotient(c), mIsSimplified(true) {}

    template<typename P = Pol, DisableIf<needs_cache<P>> = dummy>
    explicit RationalFunction(Variable v)
        : mPolynomialQuotient(std::pair<Pol, Pol>(std::move(P(v)), std::move(Pol(1)))), mNumberQuotient(), mIsSimplified(true) {}

    explicit RationalFunction(const Pol& p) : mNumberQuotient(), mIsSimplified(true) {
        if (p.isConstant())
    ...
    explicit RationalFunction(Pol&& p) : mNumberQuotient(), mIsSimplified(true) {
        if (p.isConstant())
            mNumberQuotient = p.constantPart();
        else
    ...
    explicit RationalFunction(const Pol& nom, const Pol& denom) : mNumberQuotient(), mIsSimplified(false) {
    ...
    explicit RationalFunction(Pol&& nom, Pol&& denom)
        : mPolynomialQuotient(std::pair<Pol, Pol>(std::move(nom), std::move(denom))), mNumberQuotient(), mIsSimplified(false) {
        eliminateCommonFactor(!AutoSimplify);
        assert(isConstant() || !denominatorAsPolynomial().isZero());
    }

    explicit RationalFunction(boost::optional<std::pair<Pol, Pol>>&& quotient, const CoeffType& num, bool simplified)
        : mPolynomialQuotient(std::move(quotient)), mNumberQuotient(num), mIsSimplified(simplified) {}
    ...
    RationalFunction inverse() const {
        ...
        if (isConstant()) {
            ...
            return RationalFunction(boost::optional<std::pair<Pol, Pol>>(std::pair<Pol, Pol>(mPolynomialQuotient->second, mPolynomialQuotient->first)),
                                    carl::constant_zero<CoeffType>().get(), mIsSimplified);
        }
    }
