...
class Monomial final {
   ...
   private:
     ...
     */
    explicit Monomial(Variable v, uint e = 1) : mExponents(1, std::make_pair(v, e)), mTotalDegree(e) {
        calcHash();
        assert(isConsistent());
    }
     ...
     */
    Monomial(Content&& exponents, uint totalDegree) : mExponents(std::move(exponents)), mTotalDegree(totalDegree) {
        calcHash();
        assert(isConsistent());
    }
     ...
     * @param exponents The variables and their exponents.
     */
    Monomial(const std::initializer_list<std::pair<Variable, uint>>& exponents) : mExponents(exponents) {
        std::sort(mExponents.begin(), mExponents.end(),
                  [](const std::pair<Variable, uint>& p1, const std::pair<Variable, uint>& p2) { return p1.first < p2.first; });
     ...
     * @param exponents The variables and their exponents.
     */
    explicit Monomial(Content&& exponents) : mExponents(std::move(exponents)) {
    ...
    explicit Monomial(std::size_t hash, Content exponents) : mExponents(std::move(exponents)), mHash(hash) {
        for (auto const& ve : mExponents) {
            mTotalDegree += ve.second;
        }
        assert(isConsistent());
    }
    explicit Monomial(std::size_t hash, Content exponents, uint totalDegree) : mExponents(std::move(exponents)), mTotalDegree(totalDegree), mHash(hash) {
        assert(isConsistent());
    }
   ...
   public:
    ...
    bool integerValued() const {
        auto res = std::find_if(mExponents.begin(), mExponents.end(), [](const auto& e) { return e.first.type() != VariableType::VT_INT; });
    ...
    bool isSquare() const {
        if (mTotalDegree % 2 == 1)
            return false;
        auto res = std::find_if(mExponents.begin(), mExponents.end(), [](const auto& e) { return e.second % 2 == 1; });
        return res == mExponents.end();
    }
    ...
    bool divisible(const Monomial::Arg& m) const {
        if (!m)
            return true;
        assert(isConsistent());
        if (m->mTotalDegree > mTotalDegree)
            return false;
        if (m->nrVariables() > nrVariables())
            return false;
    ...
    static CompareResult compareLexical(const Monomial::Arg& lhs, Variable rhs) {
        if (!lhs)
            return CompareResult::LESS;
        if (lhs->mExponents.front().first < rhs)
            return CompareResult::GREATER;
        if (lhs->mExponents.front().first > rhs)
            return CompareResult::LESS;
        if (lhs->mExponents.front().second > 1)
            return CompareResult::GREATER;
        return CompareResult::EQUAL;
    }
    ...
    static CompareResult compareGradedLexical(const Monomial::Arg& lhs, const Monomial::Arg& rhs) {
        ...
        if (!rhs)
            return CompareResult::GREATER;
        if (lhs->mTotalDegree < rhs->mTotalDegree)
            return CompareResult::LESS;
        if (lhs->mTotalDegree > rhs->mTotalDegree)
            return CompareResult::GREATER;
        return lexicalCompare(*lhs, *rhs);
