...
class MultivariatePolynomial : public Polynomial, public Policies {
   ...
   public:
    ...
    void setReasons(BitVector) const {
        assert(false);
    }
     ...
     * @return The sum-of-squares (sos) decomposition ((q1,p1), .., (qn,pn)) with this = q1*p1^2+..+qn*pn^2, qi being positive rational numbers and pi being
     * polynomials. If the result is empty, no sos could be found (which does not mean, that there exists no one).
     */
    ...
    size_t complexity() const {
        size_t result = 0;
        for (const auto& term : mTerms) result += term.complexity();
        return result;
    }
...
/// @}
template<typename C, typename O, typename P>
std::pair<MultivariatePolynomial<C, O, P>, MultivariatePolynomial<C, O, P>> lazyDiv(const MultivariatePolynomial<C, O, P>& _polyA,
