...
MultivariateHorner<PolynomialType, strategy>::MultivariateHorner(const PolynomialType& inPut) {
    ...
    if (strategy::use_arithmeticOperationsCounter) {
        ...
        for (polynomialIt = inPut.begin(); polynomialIt != inPut.end(); polynomialIt++) {
            ...
            if (polynomialIt->coeff() > 1)
                arithmeticOperationsCounter++;
            if (polynomialIt->isConstant())
                arithmeticOperationsCounter++;
        }
    }

    // static_assert(!(strategy::variableSelectionHeurisics == variableSelectionHeurisics::GREEDY_II)&&!(strategy::variableSelectionHeurisics ==
    // variableSelectionHeurisics::GREEDY_IIs), "Strategy requires Interval map");
    ...
    *this = root;
    ...
    if (strategy::use_arithmeticOperationsCounter) {
        std::cout << "Total AO: " << arithmeticOperationsCounter << " rAO: " << arithmeticOperationsReductionCounter << " inPut: " << inPut
...
MultivariateHorner<PolynomialType, strategy>::MultivariateHorner(const PolynomialType& inPut, const std::map<Variable, Interval<double>>& map, int& counter) {
    ...
    if (allVariablesinPolynome.size() != 0) {
        ...
        if ((*selectedVariable == NULL &&
             (strategy::selectionType == variableSelectionHeurisics::GREEDY_Is || strategy::selectionType == variableSelectionHeurisics::GREEDY_I)) ||
            (bestDelta == constant_zero<CoeffType>::get() &&
             (strategy::selectionType == variableSelectionHeurisics::GREEDY_IIs || strategy::selectionType == variableSelectionHeurisics::GREEDY_II))) {
...
#endif
            std::shared_ptr<MultivariateHorner<PolynomialType, strategy>> new_dependent(
                new MultivariateHorner<PolynomialType, strategy>(std::move(h_dependentPart), map, counter));
...
#endif
            std::shared_ptr<MultivariateHorner<PolynomialType, strategy>> new_independent(
                new MultivariateHorner<PolynomialType, strategy>(std::move(h_independentPart), map, counter));
...
std::shared_ptr<MultivariateHorner<PolynomialType, strategy>> simplify(std::shared_ptr<MultivariateHorner<PolynomialType, strategy>> mvH) {
    ...
    if (mvH->getDependent() && (mvH->getDependent()->getDependent() || mvH->getDependent()->getDepConstant() != 0) && !mvH->getDependent()->getIndependent() &&
        mvH->getDependent()->getIndepConstant() == 0) {
