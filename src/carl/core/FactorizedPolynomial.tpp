...
namespace carl {
template<typename P>
FactorizedPolynomial<P>::FactorizedPolynomial() : mCacheRef(CACHE::NO_REF), mpCache(nullptr), mCoefficient(0) {
    assert(mpCache == nullptr || mCacheRef != CACHE::NO_REF);
}

template<typename P>
FactorizedPolynomial<P>::FactorizedPolynomial(const CoeffType& _coefficient) : mCacheRef(CACHE::NO_REF), mpCache(nullptr), mCoefficient(_coefficient) {}
...
FactorizedPolynomial<P>::FactorizedPolynomial(Factorization<P>&& _factorization, const CoeffType& _coefficient, const std::shared_ptr<CACHE>& _pCache)
    : mCacheRef(CACHE::NO_REF), mpCache(_pCache), mCoefficient(_coefficient) {
    ...
    if (_factorization.empty())
        ...
        assert(mpCache != nullptr);
        // TODO expensive
        for (auto factor = _factorization.begin(); factor != _factorization.end(); factor++) assert(carl::isOne(factor->first.coefficient()));
...
FactorizedPolynomial<P>::FactorizedPolynomial(const FactorizedPolynomial<P>& _toCopy)
    : mCacheRef(_toCopy.mCacheRef), mpCache(_toCopy.mpCache), mCoefficient(_toCopy.mCoefficient) {
    if (mpCache != nullptr) {
        mpCache->reg(mCacheRef);
    }
    ASSERT_CACHE_REF_LEGAL((*this));
...
Definiteness FactorizedPolynomial<P>::definiteness(bool _fullEffort) const {
    if (existsFactorization(*this)) {
        ...
        assert(result == Definiteness::NON || computePolynomial(*this).definiteness() == Definiteness::NON ||
               result == computePolynomial(*this).definiteness());
        return result;
    }
...
template<typename P>
FactorizedPolynomial<P> FactorizedPolynomial<P>::substitute(const std::map<Variable, FactorizedPolynomial<P>>& substitutions,
                                                            const std::map<Variable, P>& substitutionsAsP) const {
    if (!existsFactorization(*this)) {
...
DivisionResult<FactorizedPolynomial<P>> FactorizedPolynomial<P>::divideBy(const FactorizedPolynomial<P>& _divisor) const {
    ...
    FactorizedPolynomial<P> r =
        dr.remainder.isConstant() ? FactorizedPolynomial<P>(dr.remainder.constantPart()) : FactorizedPolynomial<P>(dr.remainder, mpCache);
...
FactorizedPolynomial<P> operator*(const FactorizedPolynomial<P>& _lhs, const FactorizedPolynomial<P>& _rhs) {
    ...
    while (factorA != factorizationA.end() && factorB != factorizationB.end()) {
        if (factorA->first == factorB->first) {
            resultFactorization.insert(resultFactorization.end(),
                                       std::pair<FactorizedPolynomial<P>, carl::exponent>(factorA->first, factorA->second + factorB->second));
            factorA++;
            factorB++;
...
std::pair<FactorizedPolynomial<P>, FactorizedPolynomial<P>> lazyDiv(const FactorizedPolynomial<P>& _fpolyA, const FactorizedPolynomial<P>& _fpolyB) {
    assert(!_fpolyB.isZero());
    if (_fpolyA.isZero()) {
        return std::make_pair(FactorizedPolynomial<P>(), FactorizedPolynomial<P>(Coeff<P>(1)));
    }
    ...
    while (factorA != factorizationA.end() && factorB != factorizationB.end()) {
        if (factorA->first == factorB->first) {
            if (factorA->second > factorB->second)
                resultFactorizationA.insert(resultFactorizationA.end(),
                                            std::pair<FactorizedPolynomial<P>, carl::exponent>(factorA->first, factorA->second - factorB->second));
            else if (factorA->second < factorB->second)
                resultFactorizationB.insert(resultFactorizationB.end(),
                                            std::pair<FactorizedPolynomial<P>, carl::exponent>(factorA->first, factorB->second - factorA->second));
            factorA++;
            factorB++;
        } else if (factorA->first < factorB->first) {
...
template<typename P>
Factorization<P> commonDivisor(const Factorization<P>& _fFactorizationA, const Factorization<P>& _fFactorizationB, Factorization<P>& _fFactorizationRestA,
                               Factorization<P>& _fFactorizationRestB) {
...
template<typename P>
FactorizedPolynomial<P> gcd(const FactorizedPolynomial<P>& _fpolyA, const FactorizedPolynomial<P>& _fpolyB, FactorizedPolynomial<P>& _fpolyRestA,
                            FactorizedPolynomial<P>& _fpolyRestB) {
    assert(!_fpolyA.isZero() && !_fpolyB.isZero());
    _fpolyA.strengthenActivity();
    _fpolyB.strengthenActivity();
