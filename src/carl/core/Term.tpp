...
Term<Coefficient>::Term(const Coefficient& c) : mCoeff(c) {
    assert(this->isConsistent());
}
template<typename Coefficient>
Term<Coefficient>::Term(Variable v) : mCoeff(carl::constant_one<Coefficient>().get()), mMonomial(createMonomial(v, uint(1))) {
    assert(this->isConsistent());
}

template<typename Coefficient>
Term<Coefficient>::Term(Monomial::Arg m) : mCoeff(carl::constant_one<Coefficient>().get()), mMonomial(std::move(m)) {
    assert(this->isConsistent());
}
...
template<typename Coefficient>
Term<Coefficient>::Term(const Coefficient& c, Monomial::Arg m) : mCoeff(c), mMonomial(std::move(m)) {
    assert(this->isConsistent());
}

template<typename Coefficient>
Term<Coefficient>::Term(Coefficient&& c, Monomial::Arg&& m) : mCoeff(std::move(c)), mMonomial(std::move(m)) {
    assert(this->isConsistent());
}

template<typename Coefficient>
Term<Coefficient>::Term(const Coefficient& c, Variable v, uint e) : mCoeff(c), mMonomial(createMonomial(v, e)) {
    assert(this->isConsistent());
}
...
bool Term<Coefficient>::sqrt(Term<Coefficient>& res) const {
    ...
    if (!carl::sqrt_exact(this->coeff(), resCoeff))
        return false;
...
bool Term<Coefficient>::divisible(const Term& t) const {
    if (carl::remainder(this->coeff(), t.coeff()) != Coefficient(0))
        return false;
    if (this->monomial() == nullptr)
        return t.monomial() == nullptr;
    if (t.monomial() == nullptr)
...
bool operator<(const Term<Coeff>& lhs, const Monomial::Arg& rhs) {
    if (lhs.monomial() == rhs)
        return lhs.coeff() < carl::constant_one<Coeff>().get();
...
bool operator<(const Monomial::Arg& lhs, const Term<Coeff>& rhs) {
    if (lhs == rhs.monomial())
        return carl::constant_one<Coeff>().get() < rhs.coeff();
...
bool operator<(const Coeff& lhs, const Term<Coeff>& rhs) {
    if (rhs.monomial() == nullptr)
        return lhs < rhs.coeff();
    return true;
}
...
std::string Term<Coefficient>::toString(bool infix, bool friendlyVarNames) const {
    if (mMonomial) {
        if (mCoeff != CoefficientType(1)) {
            ...
            if (infix)
                return s.str() + "*" + mMonomial->toString(true, friendlyVarNames);
            else
                return "(*" + s.str() + " " + mMonomial->toString(infix, friendlyVarNames) + ")";
        } else {
            if (infix)
                return mMonomial->toString(true, friendlyVarNames);
            else
                return mMonomial->toString(infix, friendlyVarNames);
        }
    } else {
        std::stringstream s;
...
Term<Coefficient> Term<Coefficient>::gcd(const Term<Coefficient>& t1, const Term<Coefficient>& t2) {
    ...
    if (t1.isConstant() || t2.isConstant())
        return Term(Coefficient(carl::gcd(t1.coeff(), t2.coeff())));
