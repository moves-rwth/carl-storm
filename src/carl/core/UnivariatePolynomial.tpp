...
UnivariatePolynomial<Coeff>::UnivariatePolynomial(const UnivariatePolynomial& p) : mMainVar(p.mMainVar), mCoefficients(p.mCoefficients) {
    assert(this->isConsistent());
}

template<typename Coeff>
UnivariatePolynomial<Coeff>::UnivariatePolynomial(UnivariatePolynomial&& p) noexcept : mMainVar(p.mMainVar), mCoefficients() {
...
template<typename Coeff>
UnivariatePolynomial<Coeff>::UnivariatePolynomial(Variable mainVar) : mMainVar(mainVar), mCoefficients() {
    assert(this->isConsistent());
}
template<typename Coeff>
...
template<typename Coeff>
UnivariatePolynomial<Coeff>::UnivariatePolynomial(Variable mainVar, const std::vector<Coeff>& coefficients) : mMainVar(mainVar), mCoefficients(coefficients) {
    this->stripLeadingZeroes();
    assert(this->isConsistent());
}

template<typename Coeff>
UnivariatePolynomial<Coeff>::UnivariatePolynomial(Variable mainVar, std::vector<Coeff>&& coefficients) : mMainVar(mainVar), mCoefficients(coefficients) {
    this->stripLeadingZeroes();
    assert(this->isConsistent());
}

template<typename Coeff>
UnivariatePolynomial<Coeff>::UnivariatePolynomial(Variable mainVar, const std::map<uint, Coeff>& coefficients) : mMainVar(mainVar) {
...
UnivariatePolynomial<Coeff> UnivariatePolynomial<Coeff>::prem(const UnivariatePolynomial<Coeff>& divisor) const {
    ...
    if (divisor.degree() == 0)
        return UnivariatePolynomial<Coeff>(v);
 ...
 */
template<typename Coeff>
UnivariatePolynomial<Coeff> UnivariatePolynomial<Coeff>::extended_gcd(const UnivariatePolynomial& a, const UnivariatePolynomial& b, UnivariatePolynomial& s,
...
UnivariatePolynomial<Coeff> UnivariatePolynomial<Coeff>::pow(std::size_t exp) const {
    if (exp == 0)
        return UnivariatePolynomial(mMainVar, constant_one<Coeff>::get());
    if (isZero())
        return UnivariatePolynomial(mMainVar);
...
bool UnivariatePolynomial<Coeff>::divideBy(const Coeff& divisor, UnivariatePolynomial<Coeff>& quotient) const {
    ...
    if (res)
        quotient = quo.toUnivariatePolynomial(this->mainVar());
    return res;
}
...
UnivariatePolynomial<GFNumber<typename IntegralType<Coeff>::type>> UnivariatePolynomial<Coeff>::toFiniteDomain(
    const GaloisField<typename IntegralType<Coeff>::type>* galoisField) const {
...
UnivariatePolynomial<Coeff> UnivariatePolynomial<Coeff>::excludeLinearFactors(const UnivariatePolynomial<Coeff>& poly, FactorMap<Coeff>& linearFactors,
        ...
        while (true) {
            ...
            for (; shiftedTc != shiftedTcs.end(); ++shiftedTc) {
                ...
                if (maxInt / (*lcFactor) >= shiftedTc->first) {
                    ...
                    if (divisor != *tcFactor) {
                        if (!(divisor < 0 && *tcFactor < 0 && maxInt + divisor >= -(*tcFactor)) &&
                            !(divisor > 0 && *tcFactor > 0 && maxInt - divisor >= *tcFactor)) {
                            if (divisor > *tcFactor) {
                ...
                if (lcFactor == lcFactors.end()) {
                    ...
                    if (tcFactor == tcFactors.end()) {
                        ...
                        if (!carl::isOne(factor)) {
                            result *= factor;
                            CARL_LOG_TRACE("carl.core.upoly",
                                           "UnivFactor: add the factor ("
                                               << UnivariatePolynomial<Coeff>(result.mainVar(), std::initializer_list<Coeff>({(Coeff)1 / factor})) << ")^"
                                               << 1);
                            // Add the constant factor to the factors.
                            ...
                            linearFactors.insert(linearFactors.begin(),
                                                 std::pair<UnivariatePolynomial<Coeff>, uint>(
                                                     UnivariatePolynomial<Coeff>(result.mainVar(), std::initializer_list<Coeff>({factor})), 1));
                        }
                        return result;
                    }
...
Coeff UnivariatePolynomial<Coeff>::syntheticDivision(const Coeff& zeroOfDivisor) {
    ...
    if (thirdRow.front() == 0) {
        ...
        CARL_LOG_TRACE("carl.core.upoly", "UnivSynDiv: reduced by ((" << carl::abs(getDenom(thirdRow.front())) << ")*" << mainVar() << " + ("
                                                                      << (thirdRow.front() < 0 ? "-" : "") << carl::abs(getNum(thirdRow.front())) << "))  ->  "
                                                                      << *this);
        return Coeff(0);
    }
    ...
    if (characteristic<Coeff>::value != 0 && degree() >= characteristic<Coeff>::value)
        ...
        if (c.isZero()) {
            ...
            while (!z.isZero()) {
                ...
                if (numOfCpf !=
                    1)  // TODO: is this maybe only necessary because the extended_gcd returns a polynomial with non-integer coefficients but it shouldn't?
                {
                    g *= Coeff(numOfCpf);
                }
...
bool operator==(const UnivariatePolynomial<C>& lhs, const UnivariatePolynomial<C>& rhs) {
    ...
    if (lhs.mMainVar == rhs.mMainVar) {
        ...
        if (lhs.isZero() && rhs.isZero())
            return true;
        if (lhs.isConstant() && rhs.isConstant() && lhs.lcoeff() == rhs.lcoeff())
            return true;
        return MultivariatePolynomial<typename carl::UnderlyingNumberType<C>::type>(lhs) ==
               MultivariatePolynomial<typename carl::UnderlyingNumberType<C>::type>(rhs);
    }
}
template<typename C>
...
bool UnivariatePolynomial<C>::less(const UnivariatePolynomial<C>& rhs, const PolynomialComparisonOrder& order) const {
    switch (order) {
        ...
        case PolynomialComparisonOrder::LowDegree:
            if (rhs.isZero())
                return false;
            if (isZero() || this->degree() < rhs.degree())
                return true;
            return *this < rhs;
