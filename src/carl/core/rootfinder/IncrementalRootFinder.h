...
inline std::ostream& operator<<(std::ostream& os, const SplittingStrategy& s) {
    switch (s) {
        ...
        case SplittingStrategy::BINARYSAMPLE:
            return os << "BinarySample";
        case SplittingStrategy::BINARYNEWTON:
            return os << "BinaryNewton";
        case SplittingStrategy::GRID:
            return os << "Grid";
        case SplittingStrategy::EIGENVALUES:
            return os << "Eigenvalues";
        case SplittingStrategy::ABERTH:
            return os << "Aberth";
    }
}
...
class RootFinder {
   public:
     ...
     * If the root was calculated exactly, the polynomial can be divided by \f$(x - r)\f$ where \f$x\f$ is the main variable of the polynomial and \f$r\f$ is
     * the root.
...
class IncrementalRootFinder : public AbstractRootFinder<Number>, public RootFinder<Number> {
   ...
   public:
     ...
     */
    explicit IncrementalRootFinder(const UnivariatePolynomial<Number>& polynomial, const Interval<Number>& interval = Interval<Number>::unboundedInterval(),
                                   SplittingStrategy strategy = SplittingStrategy::DEFAULT, bool tryTrivialSolver = true);
