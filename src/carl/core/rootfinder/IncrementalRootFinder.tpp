...
IncrementalRootFinder<Number, C>::IncrementalRootFinder(const UnivariatePolynomial<Number>& polynomial, const Interval<Number>& interval,
                                                        SplittingStrategy strategy, bool tryTrivialSolver)
    : AbstractRootFinder<Number>(polynomial, interval, tryTrivialSolver), splittingStrategy(strategy) {
...
bool IncrementalRootFinder<Number, C>::processQueueItem() {
    ...
    switch (strategy) {
        case SplittingStrategy::GENERIC:
            splitting_strategies::GenericStrategy<Number>::getInstance()(interval, *this);
            break;
        ...
        case SplittingStrategy::BINARYSAMPLE:
            splitting_strategies::BinarySampleStrategy<Number>::getInstance()(interval, *this);
            break;
        case SplittingStrategy::BINARYNEWTON:
            splitting_strategies::BinaryNewtonStrategy<Number>::getInstance()(interval, *this);
            break;
...
void buildIsolation(std::vector<double>&& doubleRoots, const Interval<Number>& interval, RootFinder<Number>& finder) {
    ...
    for (const auto& droot : doubleRoots) {
        ...
        if (!interval.contains(n))
            continue;
        if (roots.size() > 0 && n - roots.back() < 1)
            continue;
        roots.push_back(n);
    }
    ...
    try {
        ...
        if (roots.size() == 1) {
            ...
            for (std::size_t i = 0; i < roots.size() - 1; ++i) {
                ...
                if (interval.contains(tmpSample))
                    res.push_back(tmpSample);
            }
