...
class UnivariatePolynomial : public Polynomial {
   ...
   public:
     ...
     * As the degree of the zero polynomial is \f$-\infty\f$, we assert that this polynomial is not zero. This must be checked by the caller before calling this
     * method.
     ...
     * Technically, the coprime factor is \f$ lcm(N) / gcd(D) \f$ where `N` is the set of the numerators and `D` is the set of the denominators of all
     * coefficients.
    ...
    UnivariatePolynomial negateVariable() const {
        ...
        for (std::size_t deg = 0; deg < res.coefficients().size(); deg++) {
            if (deg % 2 == 1)
                res.mCoefficients[deg] = -res.mCoefficients[deg];
        }
        return res;
    }
     ...
     */
    template<typename C = Coefficient, DisableIf<is_field<C>> = dummy, DisableIf<is_number<C>> = dummy,
             EnableIf<is_field<typename UnderlyingNumberType<C>::type>> = dummy>
     ...
     * @return `gcd(a,b)`
     */
    static UnivariatePolynomial extended_gcd(const UnivariatePolynomial& a, const UnivariatePolynomial& b, UnivariatePolynomial& s, UnivariatePolynomial& t);
    ...
    UnivariatePolynomial<GFNumber<typename IntegralType<Coefficient>::type>> toFiniteDomain(
        const GaloisField<typename IntegralType<Coefficient>::type>* galoisField) const;
    ...
    UnivariatePolynomial pseudoPrimpart() const {
        ...
        if ((c == NumberType(0)) || (c == NumberType(1)))
            return *this;
   ...
   public:
    ...
    static std::list<UnivariatePolynomial> subresultants(const UnivariatePolynomial& p, const UnivariatePolynomial& q,
    ...
    static const std::vector<UnivariatePolynomial> principalSubresultantsCoefficients(const UnivariatePolynomial& p, const UnivariatePolynomial& q,
                                                                                      SubresultantStrategy strategy = SubresultantStrategy::Default) {
        return carl::principalSubresultantsCoefficients(p, q, strategy);
    }

    [[deprecated("use carl::resultant() instead")]]
    UnivariatePolynomial<Coefficient> resultant(const UnivariatePolynomial<Coefficient>& p,
                                                SubresultantStrategy strategy = SubresultantStrategy::Default) const {
