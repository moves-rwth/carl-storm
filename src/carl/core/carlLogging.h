 ...
 * If a Filter does not contain any rule for some Channel, the parent Channel is considered. Each Filter contains a rule for the empty Channel, initialized with
 * LVL_DEFAULT.
 *
 ...
 * <ul>
 * <li>`CARLLOG_<LVL>(channel, msg)` produces a normal log message where channel should be string identifying the channel and msg is the message to be
 * logged.</li> <li>`CARLLOG_FUNC(channel, args)` produces a log message tailored for function calls. args should represent the function arguments.</li>
   ...
   public:
    ...
    virtual void configure(const Filter& f) noexcept {
        for (const auto& t : f.data()) {
            if (t.first.size() > channelwidth)
                channelwidth = t.first.size();
        }
    }
    /**
...
#define __CARL_LOG(level, channel, expr)                                                     \
    {                                                                                        \
        ...
        __ss << expr;                                                                        \
        ::carl::logging::Logger::getInstance().log(level, channel, __ss, __CARL_LOG_RECORD); \
    }
...
#define __CARL_LOG_NOFUNC(level, channel, expr)                                                     \
    {                                                                                               \
        std::stringstream __ss;                                                                     \
        __ss << expr;                                                                               \
        ::carl::logging::Logger::getInstance().log(level, channel, __ss, __CARL_LOG_RECORD_NOFUNC); \
    }
...
#define __CARL_LOG_ASSERT(channel, condition, expr) \
    if (!(condition)) {                             \
        __CARL_LOG_FATAL(channel, expr);            \
        assert(condition);                          \
    }
