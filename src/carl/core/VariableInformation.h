...
class VariableInformation<false, CoeffType> {
   ...
   public:
    ...
    explicit VariableInformation(uint degreeOfOccurence) : mMaxDegree(degreeOfOccurence), mMinDegree(degreeOfOccurence), mOccurence(1) {}

    VariableInformation(uint maxDegree, uint minDegree, uint occurence) : mMaxDegree(maxDegree), mMinDegree(minDegree), mOccurence(occurence) {}
...
class VariableInformation<true, CoeffType> : public VariableInformation<false, CoeffType> {
   ...
   public:
    VariableInformation() : VariableInformation<false, CoeffType>(), mCoeffs() {}

    explicit VariableInformation(uint degreeOfOccurence) : VariableInformation<false, CoeffType>(degreeOfOccurence), mCoeffs() {}

    VariableInformation(uint maxDegree, uint minDegree, uint occurence, std::map<uint, CoeffType>&& coeffs)
        : VariableInformation<false, CoeffType>(maxDegree, minDegree, occurence), mCoeffs(coeffs) {}

    VariableInformation(const VariableInformation<false, CoeffType>& varInfo) : VariableInformation<false, CoeffType>(varInfo), mCoeffs() {}
