...
class FactorizedPolynomial {
   public:
    ...
    friend Factorization<P1> gcd(const PolynomialFactorizationPair<P1>& _pfPairA, const PolynomialFactorizationPair<P1>& _pfPairB, Factorization<P1>& _restA,
                                 Factorization<P1>& _rest2B, bool& _pfPairARefined, bool& _pfPairBRefined);
...
#define ASSERT_CACHE_REF_LEGAL(_fp) assert((_fp.pCache() == nullptr) == (_fp.cacheRef() == CACHE::NO_REF))
     ...
     */
    template<typename P1>
    friend Factorization<P1> commonDivisor(const FactorizedPolynomial<P1>& _fFactorizationA, const FactorizedPolynomial<P1>& _fFactorizationB,
                                           Factorization<P1>& _fFactorizationRestA, Factorization<P1>& _fFactorizationRestB);
     ...
     */
    template<typename P1>
    friend FactorizedPolynomial<P1> gcd(const FactorizedPolynomial<P1>& _fpolyA, const FactorizedPolynomial<P1>& _fpolyB, FactorizedPolynomial<P1>& _fpolyRestA,
   ...
   public:
    ...
    bool isConsistent() const {
        return polynomial().isConsistent();
    }
    ...
    size_t size() const {
        if (existsFactorization(*this)) {
            ...
            size_t result = 0;
            for (const auto& factor : content().factorization()) result += factor.first.size();
            return result;
        }
        return 1;
    }
    ...
    size_t complexity() const {
        if (existsFactorization(*this)) {
            ...
            size_t result = 0;
            for (const auto& factor : content().factorization()) result += factor.first.complexity();
            return result;
        }
        return 1;
    }
     ...
     */
    template<typename P1>
    friend std::pair<FactorizedPolynomial<P1>, FactorizedPolynomial<P1>> lazyDiv(const FactorizedPolynomial<P1>& _fpolyA,
...
template<typename P>
struct needs_cache<FactorizedPolynomial<P>> : std::true_type {};

template<typename P>
struct is_factorized<FactorizedPolynomial<P>> : std::true_type {};
