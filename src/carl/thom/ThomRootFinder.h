...
template<typename Number>
std::list<ThomEncoding<Number>> realRootsThom(const MultivariatePolynomial<Number>& p, Variable::Arg mainVar, std::shared_ptr<ThomEncoding<Number>> point_ptr,
 ...
 */
template<typename Number>
std::list<ThomEncoding<Number>> realRootsThom(const MultivariatePolynomial<Number>& p, Variable::Arg mainVar,
...
std::list<ThomEncoding<Number>> realRootsThom(const MultivariatePolynomial<Number>& p, Variable::Arg mainVar, std::shared_ptr<ThomEncoding<Number>> point_ptr,
    ...
    if (point_ptr == nullptr) {
        ...
        for (const auto& sigma : signs) {
            ThomEncoding<Number> newEncoding(sigma, p, mainVar, nullptr, sd_ptr, sigma.size());
        ...
        for (const auto& sigma : signs) {
            ...
            if (pointSigns.isSuffixOf(sigma)) {
                ...
                ThomEncoding<Number> newEncoding(newSigma, p, mainVar, point_ptr, sd_ptr, relevant);
                result.push_back(newEncoding);
            }
        }
    }
...
std::list<RealAlgebraicNumber<Number>> realRootsThom(const UnivariatePolynomial<Coeff>& p, const std::map<Variable, RealAlgebraicNumber<Number>>& m,
                                                     const Interval<Number>& interval) {
    ...
    if (tmp.gatherVariables().size() <= 1) {
        if (tmp.isZero())
            return {RealAlgebraicNumber<Number>(0)};
        assert(tmp.gatherVariables().size() == 1);
        // Coeff = MultivariatePolynomial<Number>, but all coefficients of tmp are numerical
        UnivariatePolynomial<Number> tmp_univ = tmp.convert(std::function<Number(const Coeff&)>([](const Coeff& c) {
            assert(c.isUnivariate());
            return c.constantPart();
        }));
