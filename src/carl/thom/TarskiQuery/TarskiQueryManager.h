   ...
   public:
    ...
    QueryResultType operator()(const Polynomial& p) const {
        ...
        if (this->isUnivariateManager()) {
            ...
            if (p.isConstant())
                pUniv = UnivariatePolynomial<Number>(mZ.mainVar(), p.lcoeff());
            else
                pUniv = p.toUnivariatePolynomial();
            CARL_LOG_ASSERT("carl.thom.tarski.manager", pUniv.mainVar() == mZ.mainVar(), "cannot compute tarski query of " << p << " on " << mZ);
