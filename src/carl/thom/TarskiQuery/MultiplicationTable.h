...
class MultiplicationTable {
   ...
   public:
    ...
    typename std::unordered_map<Monomial, TableContent>::const_iterator begin() const {
        return mTable.cbegin();
    }
    typename std::unordered_map<Monomial, TableContent>::const_iterator end() const {
        return mTable.cend();
    }
    typename std::unordered_map<Monomial, TableContent>::const_iterator cbegin() const {
        return mTable.cbegin();
    }
    typename std::unordered_map<Monomial, TableContent>::const_iterator cend() const {
        return mTable.cend();
    }
        ...
        for (uint i = 0; i < mBase.size(); i++) {
            for (uint j = i; j < mBase.size(); j++) {
                if (mBase[i] * mBase[j] == c) {
                    ...
                    if (i != j)
                        res.push_front(std::make_pair(j, i));
                }
            }
        }
        return res;
