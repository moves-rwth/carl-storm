...
class SignDetermination {
   ...
   public:
    ...
    SignDetermination(InputIt zeroSet_first, InputIt zeroSet_last) : mTaQ(zeroSet_first, zeroSet_last) {}
   ...
   private:
    ...
    std::list<Alpha> firstNLines(const uint n, const Eigen::MatrixXf& mat, const std::vector<Alpha>& ada, std::vector<Polynomial>& products,
                                 const uint q) const {
    ...
    void update() {
        ...
        if (mSigns.size() != r1) {
            ...
            std::list<Alpha> A_2 = firstNLines(r2, m2, std::vector<Alpha>(mAda.begin(), mAda.end()), products, 1);
            for (auto& alpha : A_2) {
                alpha.push_front(1);
            ...
            if (mSigns.size() != r1 + r2) {
                ...
                std::list<Alpha> A_3 = firstNLines(r3, m3, std::vector<Alpha>(mAda.begin(), mAda.end()), products, 2);
                for (auto& alpha : A_3) {
                    alpha.push_front(2);
                    newAda.push_back(alpha);
                }
    ...
    std::list<SignCondition> getSigns(const Polynomial& p, std::list<Polynomial>& products, std::list<Alpha>& ada, std::list<uint>& adaHelper,
        ...
        if (czer != 0)
            currSigns.emplace_back(1, Sign::ZERO);
        if (cpos != 0)
            currSigns.emplace_back(1, Sign::POSITIVE);
        if (cneg != 0)
            currSigns.emplace_back(1, Sign::NEGATIVE);
        currAda = {{0}, {1}, {2}};
