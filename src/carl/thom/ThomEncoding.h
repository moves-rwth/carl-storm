...
class ThomEncoding {
   ...
   public:
    ThomEncoding(SignCondition sc, const Polynomial& p, Variable mainVar, std::shared_ptr<ThomEncoding<Number>> point,
                 std::shared_ptr<SignDetermination<Number>> sd, uint mRelevant)
        : mSc(std::move(sc)), mP(p), mMainVar(mainVar), mPoint(std::move(point)), mSd(std::move(sd)), mRelevant(mRelevant) {}

    ThomEncoding(const Number& n, Variable mainVar, std::shared_ptr<ThomEncoding<Number>> point = nullptr) : mMainVar(mainVar), mPoint(std::move(point)) {
    ...
    inline SignCondition signCondition() const {
        return mSc;
    }
    inline SignCondition relevantSignCondition() const {
        return mSc.trailingPart(mRelevant);
    }
    inline Variable::Arg mainVar() const {
        return mMainVar;
    }
    inline const Polynomial& polynomial() const {
        return mP;
    }
    inline const ThomEncoding<Number>& point() const {
        assert(mPoint);
        return *mPoint;
    }
    inline SignDetermination<Number> sd() const {
        assert(mSd);
        return *mSd;
    }
    ...
    ThomEncoding<Number> lowestInChain() const {
        if (mPoint == nullptr)
            return ThomEncoding<Number>(*this);
    ...
    SignCondition accumulateRelevantSigns() const {
        if (mPoint == nullptr)
            return this->relevantSignCondition();
    ...
    bool makesPolynomialZero(const Polynomial& pol, Variable::Arg pol_mainVar) const {
        ...
        for (const auto& t : pol_univ.coefficients()) {
            ...
            if (this->signOnPolynomial(t) != Sign::ZERO)
                return false;
        }
        return true;
    }
    ...
    static ThomEncoding<Number> analyzeTEMap(const std::map<Variable, ThomEncoding<Number>>& m) {
        ...
        encodings.sort([](const ThomEncoding<Number>& lhs, const ThomEncoding<Number>& rhs) { return lhs.dimension() > rhs.dimension(); });
   ...
   public:
    ...
    bool equals(const ThomEncoding<Number>& other) const {
        if (this->polynomial() != other.polynomial())
            return false;
        if (this->relevantSignCondition() != other.relevantSignCondition())
            return false;
   ...
   public:
    ...
    ThomEncoding<Number> operator+(const Number& rhs) const {
        ...
        for (const auto& r : roots) {
            if (relevantSignCondition() == r.mSc)
                return r;
        }
...
bool operator<(const ThomEncoding<N>& lhs, const ThomEncoding<N>& rhs) {
    return ThomEncoding<N>::compare(lhs, rhs) == LESS;
}
template<typename N>
bool operator<=(const ThomEncoding<N>& lhs, const ThomEncoding<N>& rhs) {
    return ThomEncoding<N>::compare(lhs, rhs) != GREATER;
}
template<typename N>
bool operator>(const ThomEncoding<N>& lhs, const ThomEncoding<N>& rhs) {
    return ThomEncoding<N>::compare(lhs, rhs) == GREATER;
...
template<typename N>
bool operator==(const ThomEncoding<N>& lhs, const ThomEncoding<N>& rhs) {
    return ThomEncoding<N>::compare(lhs, rhs) == EQUAL;
}
template<typename N>
bool operator!=(const ThomEncoding<N>& lhs, const ThomEncoding<N>& rhs) {
    return ThomEncoding<N>::compare(lhs, rhs) != EQUAL;
...
bool operator<(const ThomEncoding<N>& lhs, const N& rhs) {
    return ThomEncoding<N>::compareRational(lhs, rhs) == LESS;
}
template<typename N>
bool operator<=(const ThomEncoding<N>& lhs, const N& rhs) {
    return ThomEncoding<N>::compareRational(lhs, rhs) != GREATER;
}
template<typename N>
bool operator>(const ThomEncoding<N>& lhs, const N& rhs) {
    return ThomEncoding<N>::compareRational(lhs, rhs) == GREATER;
...
template<typename N>
bool operator==(const ThomEncoding<N>& lhs, const N& rhs) {
    return ThomEncoding<N>::compareRational(lhs, rhs) == EQUAL;
}
template<typename N>
bool operator!=(const ThomEncoding<N>& lhs, const N& rhs) {
    return ThomEncoding<N>::compareRational(lhs, rhs) != EQUAL;
