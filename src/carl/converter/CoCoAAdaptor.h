...
class CoCoAAdaptor {
   ...
   public:
    explicit CoCoAAdaptor(const std::vector<Variable>& vars) : mSymbolBack(vars), mRing(CoCoA::NewPolyRing(mQ, long(mSymbolBack.size()))) {
    ...
    CoCoAAdaptor(const std::vector<Poly>& polys) : CoCoAAdaptor(collectVariables(polys)) {}
    CoCoAAdaptor(const std::initializer_list<Poly>& polys) : CoCoAAdaptor(std::vector<Poly>(polys)) {}
