   ...
   public:
    ...
    GiNaC::ex convertToGinac(const P& poly, const std::map<carl::Variable, GiNaC::ex>& vars) {
        ...
        if (existsFactorization(poly)) {
            ...
            GiNaC::ex result =
                GiNaC::ex(GiNaC::numeric(carl::rationalize<cln::cl_RA>(PreventConversion<typename Poly::PolyType::CoeffType>(poly.coefficient()))));
    ...
    void gatherVariables(const Poly& poly, std::map<Variable, GiNaC::ex>& carlToGinacVarMap,
