...
class Buchberger : private AddingPolicy<Polynomial> {
   ...
   public:
    Buchberger() : pGb(), mGbElementsIndices(), pCritPairs(new CritPairs()), mUpdateCallBack(this) {}

    virtual ~Buchberger() = default;

    Buchberger(const Buchberger& rhs)
        : pGb(new Ideal<Polynomial>(*rhs.pGb)), mGbElementsIndices(rhs.mGbElementsIndices), pCritPairs(new CritPairs(*rhs.pCritPairs)), mUpdateCallBack(this) {}
