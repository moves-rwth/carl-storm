 ...
 * @ingroup gb
 */
template<typename InputPolynomial, typename PolynomialInIdeal, template<class> class Datastructure = carl::Heap,
         template<typename Polynomial> class Configuration = ReductorConfiguration>
class Reductor {
   protected:
   ...
   public:
    ...
    Reductor(const Ideal<PolynomialInIdeal>& ideal, const Term<Coeff>& f) : mIdeal(ideal), mDatastruct(Configuration<InputPolynomial>()) {
        insert(f);
    }
