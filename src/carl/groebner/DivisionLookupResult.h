...
struct DivisionLookupResult {
    DivisionLookupResult() : mDivisor(nullptr), mFactor() {}

    DivisionLookupResult(const DivisionLookupResult& d) : mDivisor(d.mDivisor), mFactor(d.mFactor) {}

    virtual ~DivisionLookupResult() {
        // intentionally left empty
    }

    DivisionLookupResult(const Polynomial* divisor, const Term<typename Polynomial::CoeffType>& factor) : mDivisor(divisor), mFactor(factor) {}
