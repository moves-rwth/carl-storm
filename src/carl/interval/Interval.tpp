...
Sign Interval<Number>::sgn() const {
    ...
    if (this->isInfinite())
        return Sign::ZERO;
    if ((mLowerBoundType == BoundType::STRICT && mContent.lower() >= carl::constant_zero<Number>().get()) ||
        (mLowerBoundType == BoundType::WEAK && mContent.lower() > carl::constant_zero<Number>().get()))
        return Sign::POSITIVE;
    if ((mUpperBoundType == BoundType::STRICT && mContent.upper() <= carl::constant_zero<Number>().get()) ||
        (mUpperBoundType == BoundType::WEAK && mContent.upper() < carl::constant_zero<Number>().get()))
...
bool Interval<Number>::containsInteger() const {
    ...
    switch (mLowerBoundType) {
        ...
        case BoundType::STRICT:
            break;
        case BoundType::WEAK:
            if (carl::isInteger(mContent.lower()))
                return true;
    }
    switch (mUpperBoundType) {
        case BoundType::INFTY:
        ...
        case BoundType::STRICT:
            break;
        case BoundType::WEAK:
            if (carl::isInteger(mContent.upper()))
                return true;
    }
...
bool Interval<Number>::contains(const Number& val) const {
    ...
    return true;  // for open intervals: (lower() < n && upper() > n) || (n == carl::constant_zero<Number>().get() && lower() == cln::cl_RA( 0 ) && upper() ==
                  // cln::cl_RA( 0 ))	}
}
...
std::pair<Interval<Number>, Interval<Number>> Interval<Number>::split() const {
    ...
    return std::pair<Interval<Number>, Interval<Number>>(Interval(bisection.first, mLowerBoundType, BoundType::STRICT),
                                                         Interval(bisection.second, BoundType::WEAK, mUpperBoundType));
}
...
Interval<Number> Interval<Number>::add(const Interval<Number>& rhs) const {
    ...
    return Interval<Number>(mContent + rhs.content(), getStrictestBoundType(mLowerBoundType, rhs.lowerBoundType()),
...
bool Interval<Number>::div_ext(const Interval<Number>& rhs, Interval<Number>& a, Interval<Number>& b) const {
    ...
    if (rhs.lowerBoundType() != BoundType::INFTY && rhs.lower() == carl::constant_zero<Number>().get() && rhs.upperBoundType() != BoundType::INFTY &&
        rhs.upper() == carl::constant_zero<Number>().get())  // point interval 0
    {
        splitOccured = false;
...
bool Interval<Number>::reciprocal(Interval<Number>& a, Interval<Number>& b) const {
    if (this->isInfinite()) {
        a = emptyInterval();
        return false;
    } else if (this->contains(carl::constant_zero<Number>().get()) && mContent.lower() != carl::constant_zero<Number>().get() &&
               mContent.upper() != carl::constant_zero<Number>().get()) {
...
Interval<Number> Interval<Number>::sqrt() const {
    ...
    if (mLowerBoundType == BoundType::INFTY || mContent.lower() < carl::constant_zero<Number>().get()) {
        if (mUpperBoundType == BoundType::INFTY) {
            ...
            return Interval<Number>(boost::numeric::sqrt(BoostInterval(carl::constant_zero<Number>().get(), mContent.upper())), BoundType::WEAK,
                                    mUpperBoundType);
        }
    }
    ...
    if (deg % 2 == 0) {
        ...
        if (mLowerBoundType == BoundType::INFTY || mContent.lower() < carl::constant_zero<Number>().get()) {
            if (mUpperBoundType == BoundType::INFTY) {
                ...
                return Interval<Number>(boost::numeric::nth_root(BoostInterval(carl::constant_zero<Number>().get(), mContent.upper()), deg), BoundType::WEAK,
                                        mUpperBoundType);
            }
        }
    }
...
inline bool operator==(const Interval<Number>& lhs, const Interval<Number>& rhs) {
    return lhs.content().lower() == rhs.content().lower() && lhs.content().upper() == rhs.content().upper() && lhs.lowerBoundType() == rhs.lowerBoundType() &&
           lhs.upperBoundType() == rhs.upperBoundType();
}
