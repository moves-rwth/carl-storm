...
struct is_interval<carl::Interval<Number>> : std::true_type {};
/**
 * States that `const boost::variant` is indeed a `boost::variant`.
 */
template<class Number>
struct is_interval<const carl::Interval<Number>> : std::true_type {};
...
struct policies<double, Interval> {
    using roundingP = boost::numeric::interval_lib::save_state<boost::numeric::interval_lib::rounded_transc_std<
        double>>;  // TODO: change it to boost::numeric::interval_lib::rounded_transc_opp, if new boost release patches the bug with clang
...
#define IS_EMPTY(lower, lowerBoundType, upper, upperBoundType)                        \
    (((lowerBoundType == BoundType::STRICT && upperBoundType != BoundType::INFTY) ||  \
      (lowerBoundType != BoundType::INFTY && upperBoundType == BoundType::STRICT)) && \
     lower == upper)
...
#define IS_UNBOUNDED(lower, lowerBoundType, upper, upperBoundType) (lowerBoundType == BoundType::INFTY && upperBoundType == BoundType::INFTY)
   ...
   public:
     ...
     */
    Interval() : mContent(carl::constant_zero<Number>().get()) {}
     ...
     */
    Interval(const Number& lower, const Number& upper) : mContent(), mLowerBoundType(), mUpperBoundType() {
     ...
     */
    Interval(const Number& lower, BoundType lowerBoundType, const Number& upper, BoundType upperBoundType) : mContent(), mLowerBoundType(), mUpperBoundType() {
     ...
     */
    Interval(const Interval<Number>& o) : mContent(BoostInterval(o.mContent)), mLowerBoundType(o.mLowerBoundType), mUpperBoundType(o.mUpperBoundType) {}
    ...
    explicit Interval(const double& n)
        : mContent(carl::Interval<Number>::BoostInterval(n, n)), mLowerBoundType(BoundType::WEAK), mUpperBoundType(BoundType::WEAK) {
        Policy::sanitize(*this);
    }
    ...
    explicit Interval(Rational n) : mContent(), mLowerBoundType(), mUpperBoundType() {
     ...
     */
    template<typename Num = Number, typename Rational, EnableIf<std::is_floating_point<Num>> = dummy, DisableIf<std::is_same<Num, Rational>> = dummy>
    Interval(Rational lower, Rational upper) : mContent(), mLowerBoundType(), mUpperBoundType() {
    ...
    Interval(Rational lower, BoundType lowerBoundType, Rational upper, BoundType upperBoundType) {
        ...
        if (dLeft == -std::numeric_limits<double>::infinity())
            mLowerBoundType = BoundType::INFTY;
        if (dRight == std::numeric_limits<double>::infinity())
            mUpperBoundType = BoundType::INFTY;
     ...
     * @param n The passed double.
     */
    template<typename Num = Number, typename Float, EnableIf<is_rational<Num>> = dummy, EnableIf<std::is_floating_point<Float>> = dummy,
             DisableIf<std::is_same<Num, Float>> = dummy>
    explicit Interval(Float n) : mContent(), mLowerBoundType(), mUpperBoundType() {
        *this = Interval<double>(n, n);
        Policy::sanitize(*this);
    }
    ...
    template<typename Num = Number, typename Float, EnableIf<is_rational<Num>> = dummy, EnableIf<std::is_floating_point<Float>> = dummy,
             DisableIf<std::is_same<Num, Float>> = dummy>
    Interval(Float lower, Float upper) : mContent(), mLowerBoundType(), mUpperBoundType() {
        *this = Interval<double>(lower, BoundType::WEAK, upper, BoundType::WEAK);
        Policy::sanitize(*this);
    }
     ...
     */
    template<typename Num = Number, typename Float, EnableIf<is_rational<Num>> = dummy, EnableIf<std::is_floating_point<Float>> = dummy,
             DisableIf<std::is_same<Num, Float>> = dummy, DisableIf<std::is_floating_point<Num>> = dummy>
     ...
     * @param n The passed double.
     */
    template<typename Num = Number, typename Rational, EnableIf<is_rational<Num>> = dummy, EnableIf<is_rational<Rational>> = dummy,
             DisableIf<std::is_same<Num, Rational>> = dummy>
    explicit Interval(Rational n) : mContent(), mLowerBoundType(), mUpperBoundType() {
        *this = Interval<double>(n, n);
        Policy::sanitize(*this);
    }
    ...
    inline bool isEmpty() const {
        ...
        if (mContent.lower() == mContent.upper()) {
            if (mLowerBoundType == BoundType::STRICT && mUpperBoundType == BoundType::STRICT)
                return true;
            if (mLowerBoundType == BoundType::STRICT && mUpperBoundType == BoundType::WEAK)
                return true;
            if (mLowerBoundType == BoundType::WEAK && mUpperBoundType == BoundType::STRICT)
                return true;
        }
        return false;
    }
    ...
    N center() const {
        assert(this->isConsistent());
        if (this->isInfinite())
            return carl::constant_zero<N>().get();
        if (this->mLowerBoundType == BoundType::INFTY)
...
struct hash<carl::Interval<Number>> {
    ...
    std::size_t operator()(const carl::Interval<Number>& interval) const {
        return carl::hash_all(interval.lowerBoundType(), interval.upperBoundType(), interval.lower(), interval.upper());
    }
};
}  // namespace std
