...
# Settings
auto_format_file_extensions=( .h .cpp .tpp )
auto_format_src_dir=./src

# Set-up directories
script_dir=$( cd -- "$( dirname -- "${BASH_SOURCE[0]}" )" &> /dev/null && pwd )
carl_root="$script_dir/../.."

# Sanity checks
for expected_file in .clang-format .clang-format-ignore
do
	if [ ! -f $carl_root/$expected_file ]; then
    	echo "ERROR: There does not seem to be a file '$carl_root/$expected_file'. Have you moved this script?"
		exit 1
	fi
done
